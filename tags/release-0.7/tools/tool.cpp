...
Tool::Tool( ToolId id, SettingNotifier& notifier, ToolSettingMode settingMode )
  : m_id(id),
    m_relay( notifier ),
    m_settingEater( settingMode==EAT_SETTINGS )
{}
...
const Settings& Tool::GetSettings() const{
  return m_settings;
}
...
ToolResult Tool::LeftDoubleClick( const CursorPositionInfo& info ){
  // Default to forwarding the double click as another left down
  return LeftDown(info);
}
