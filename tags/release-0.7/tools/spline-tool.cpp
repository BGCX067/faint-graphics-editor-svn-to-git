...
ToolResult SplineTool::Commit(Layer::type layerType){
...
bool SplineTool::Draw( FaintDC& dc, Overlays&, const Point& currPos ){
  if ( m_active ){
...
bool SplineTool::DrawBeforeZoom(Layer::type layer) const{
...
Cursor::type SplineTool::GetCursor( const CursorPositionInfo& ) const{
...
IntRect SplineTool::GetRefreshRect( const IntRect&, const Point& currPos ) const{
  return truncated( inflated( bounding_rect( m_p1, m_p2, currPos ), m_settings.Get( ts_LineWidth ) ) );
}
