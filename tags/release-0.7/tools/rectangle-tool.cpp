...
Point rect_constrain( const Point& p0, const Point& p1, bool subPixel ){
  return subPixel? adjust_to(p0, p1, 90, 45 ):
    adjust_to( floated(truncated(p0)), floated(truncated(p1)), 90, 45 );
}
...
bool RectangleTool::Draw( FaintDC& dc, Overlays&, const Point& ){
  if ( !m_active ){
    return false;
  }
...
bool RectangleTool::DrawBeforeZoom(Layer::type layer) const{
...
Cursor::type RectangleTool::GetCursor(const CursorPositionInfo&) const{
