...
int Application::FilterEvent( wxEvent& untypedEvent ){
  ...
  if ( !boundGlobal ){
    EntryMode::Type entryMode = m_frame->GetTextEntryMode();
    if ( entryMode == EntryMode::ALPHA_NUMERIC ){
      // Any Python bind might interfere with alpha-numeric text
      // entry. (Fixme: Should still allow F-keys and some modified keys)
      return Event_Skip;
    }
...
void Application::LoadGraphics(){
  ...
  FileAndLabel files[] = {
    ...
    {"graphics/close-frame.png", "close_frame"},
...
bool Application::OnCmdLineParsed(wxCmdLineParser& parser){
  ...
  if ( !valid_port(m_cmd.port) ){
    faint::console_message(no_sep("Error: Invalid port specified ",
        "(\"",m_cmd.port, "\")"));
    return false;
  }
