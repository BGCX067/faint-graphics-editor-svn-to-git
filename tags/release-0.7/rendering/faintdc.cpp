...
void FaintDC::DrawRasterEllipse( const Tri& tri, const Settings& s ){
  ...
  if ( border(s) ){
    draw_ellipse( m_bitmap, r, truncated(s.Get( ts_LineWidth )), get_color_fg(s), dashed(s) );
  }
}
...
void FaintDC::Ellipse( const Tri& tri, const Filter& f, const Settings& s ){
  ...
  faint::Bitmap bmp(faint::cairo_compatible_bitmap(r.GetSize() + extraSize));
  IntPoint offset(10,10);
  faint::clear(bmp,faint::Color(255,255,255,0));
  IntRect r2(translated(r, -r.TopLeft() + offset));
  if ( filled(s) ){
    fill_ellipse( bmp, r2, get_color_bg(s) );
  }
  if ( border(s) ){
    draw_ellipse( bmp, r2, truncated(s.Get( ts_LineWidth )), get_color_fg(s), dashed(s));
  }
  f.Apply(bmp);
...
void FaintDC::Line( const Point& in_p0, const Point& in_p1, const Filter& f, const Settings& s ){
  IntSize extraSize(40,40);
  faint::Bitmap bmp(faint::cairo_compatible_bitmap(truncated(Size(fabs(in_p1.x - in_p0.x), fabs(in_p1.y - in_p0.y))) + extraSize));
  IntPoint origin = truncated(min_coords(in_p0, in_p1));
  IntPoint offset(10,10);
  faint::clear(bmp,faint::Color(255,255,255,0));
  faint::draw_line(bmp, truncated(in_p0) - origin + offset, truncated(in_p1) - origin + offset, s.Get(ts_FgCol),
    truncated(s.Get(ts_LineWidth)), dashed(s), get_line_cap(s));
  f.Apply(bmp);
...
void FaintDC::Rectangle( const Tri& tri, const Filter& f, const Settings& s ){
  ...
  IntRect r2(translated(r, -r.TopLeft() + offset));
  if ( filled(s) ){
    fill_rect( bmp, r2, get_color_bg(s) );
  }
  if ( border(s) ){
    draw_rect( bmp, r2, get_color_fg(s),
      truncated(s.Get( ts_LineWidth ) * m_sc),
      dashed(s) );
  }
  f.Apply(bmp);
...
std::vector<int> FaintDC::PartialExtents( const faint::utf8_string& text, const Settings& s ) const{
  return m_cr->partial_extents(text, s);
}
