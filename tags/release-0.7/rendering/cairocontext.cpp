...
namespace faint{
faint::Bitmap cairo_compatible_bitmap( const IntSize& sz ){
...
faint::Bitmap cairo_compatible_sub_bitmap( const Bitmap& orig, const IntRect& r ){
...
faint::Bitmap cairo_skew( faint::Bitmap& bmpSrc, faint::coord skew_angle, faint::coord skew_pixels ){
...
Size CairoContext::pango_text_extents( const utf8_string& text, const Settings& s ) const{
  ...
  //utf8_string adjusted = faint::replace_chars(text, faint::space, faint::no_break_space);
  pango_layout_set_text(layout, text.c_str(), -1);
...
std::vector<int> CairoContext::partial_extents( const utf8_string& text, const Settings& s ) const{
  const bool bold = s.Has( ts_FontBold ) && s.Get( ts_FontBold );
  const bool italic = s.Has( ts_FontItalic ) && s.Get( ts_FontItalic );
  PangoFontDescription* font_description = pango_font_description_new();
  pango_font_description_set_family(font_description, s.Get( ts_FontFace ).c_str());
