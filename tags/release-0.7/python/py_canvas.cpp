...
#include "commands/add-frame-cmd.hh"
...
#include "commands/set-frame-delay-cmd.hh"
...
static PyObject* canvas_frame_set_delay( canvasObject* self, PyObject* args ){
  ...
  self->canvas->RunCommand(set_frame_delay_command(index_t(num), New(newDelay), Old(oldDelay)));
  return Py_BuildValue("");
}
...
static PyObject* canvas_dumb_ellipse(canvasObject* self, PyObject* args ){
  if ( !canvas_ok( self->id ) ){
    return canvas_removed_error();
  }
  ...
  IntRect rect;
  if ( !parse_intrect(args, &rect ) ){
    PyErr_SetString( PyExc_TypeError, "Invalid parameters to dumb_ellipse(x0, y0, w, h)");
    return 0;
  }
  Settings s( default_ellipse_settings() );
  s.Update( GetAppContext().GetToolSettings() );
  Command* cmd = get_dumb_ellipse_command(rect, static_cast<int>(s.Get(ts_LineWidth)), s.Get(ts_FgCol));
  faint::python_run_command(self->canvas, cmd);
  return Py_BuildValue("");
}
...
static PyObject* canvas_aa_line(canvasObject* self, PyObject* args ){
  ...
  s.Update( GetAppContext().GetToolSettings() );
  Command* cmd = get_aa_line_command(truncated(p0), truncated(p1), s.Get(ts_FgCol));
  faint::python_run_command(self->canvas, cmd);
  return Py_BuildValue("");
}
...
static PyMethodDef canvas_methods[] = {
  ...
  {"dumb_ellipse", (PyCFunction)canvas_dumb_ellipse, METH_VARARGS,
   "dumb_ellipse(x0,y0,w,h)\nExperimental!\nDraw a dumb ellipse bounded by the given rectangle"},
...
static int canvas_init(canvasObject* self, PyObject* args, PyObject* ){
  ...
  const int n = PySequence_Length( args );
  if ( faint::is_string_sequence(args) && n > 1 ){
