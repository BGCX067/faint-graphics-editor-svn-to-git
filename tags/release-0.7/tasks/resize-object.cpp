...
TaskResult maybe_set_resize_command( bool copy, PendingCommand& cmd, Object* object, const Tri& oldTri ){
  object->ClearActive();
  if ( !copy ){
...
ResizeObjectTask::ResizeObjectTask(Object* object, int handleIndex, MoveMode::Type mode )
...
bool ResizeObjectTask::Draw( FaintDC& dc, Overlays& overlays, const Point& ){
  if ( m_copy ){
    m_object->Draw(dc);
  }
...
bool ResizeObjectTask::DrawBeforeZoom(Layer::type) const {
  return false;
}
...
Cursor::type ResizeObjectTask::GetCursor(const CursorPositionInfo&) const{
  return Cursor::ARROW;
}

IntRect ResizeObjectTask::GetRefreshRect( const IntRect&, const Point& ) const{
  return inflated(m_object->GetRefreshRect(), 5); // Fixme: Inflate is for clearing handles, use a constant or full-refresh
}
...
TaskResult ResizeObjectTask::Motion( const CursorPositionInfo& info ){
  ...
  faint::coord scale_x = m_lockX ? 1.0 : ( p1.x - m_origin.x ) / ( m_p0.x - m_origin.x );
  faint::coord scale_y = m_lockY ? 1.0 : ( p1.y - m_origin.y ) / ( m_p0.y - m_origin.y );
