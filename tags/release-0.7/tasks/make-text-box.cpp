...
bool MakeTextBox::Draw( FaintDC&, Overlays& overlays, const Point& ){
  overlays.Rectangle( Rect(m_p0, m_p1) );
  return true;
}

bool MakeTextBox::DrawBeforeZoom( Layer::type ) const{
  return false;
}
...
Cursor::type MakeTextBox::GetCursor( const CursorPositionInfo& ) const{
