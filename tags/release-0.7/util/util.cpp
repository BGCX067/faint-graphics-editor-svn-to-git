...
IntPoint truncate_point(const Point& pt ){ // Boo! Required, or std::transform could not deduce overload.
  return truncated(pt);
}

std::vector<IntPoint> truncated( const std::vector<Point>& points ){
  std::vector<IntPoint> v2;
  v2.reserve(points.size());
  std::transform(points.begin(), points.end(), std::back_inserter(v2), truncate_point );
  return v2;
}
...
std::vector<Point> transform_points( const std::vector<Point>& points, faint::coord scale, const Point& origin ){
  ...
  for ( size_t i = 0; i != points.size(); i++ ){
    v2.push_back( points[i] * scale + origin );
  }
  return v2;
}
