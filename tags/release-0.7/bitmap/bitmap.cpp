...
bool operator==(const color_ptr& c1, const faint::Color& c2 ){
  return c1.r == c2.r &&
    c1.g == c2.g &&
    c1.b == c2.b &&
    c1.a == c2.a;
...
faint::Bitmap alpha_blended( const Bitmap& src, const Color& color ){
...
void draw_wide_ellipse( faint::Bitmap& bmp, const IntRect& r, int lineWidth, const faint::Color& color ){
  ...
  void draw_ellipse( faint::Bitmap& bmp, const IntRect& r, int lineWidth, const faint::Color& c, bool dashes ){
  if ( lineWidth > 1 ){
  ...
  if ( a == 0 ){
    draw_line( bmp, r.TopLeft(), r.BottomLeft(), c, 1, dashes, LineCap::BUTT );
    return;
  }
  if ( b == 0 ){
    draw_line( bmp, r.TopLeft(), r.TopRight(), c, 1, dashes, LineCap::BUTT );
    return;
  }
...
faint::Bitmap flip_horizontal( const Bitmap& src ){
  Bitmap dst( src );
...
faint::Bitmap flip_vertical( const Bitmap& src ){
  Bitmap dst( src );
  uchar* pDst = dst.m_data;
  uchar* pSrc = src.m_data;
  for ( uint y = 0; y != src.m_h; y++ ){
...
faint::Bitmap flip( const Bitmap& bmp, Axis::type axis ){
...
faint::Bitmap get_null_bitmap(){
  faint::Bitmap nullBitmap = faint::Bitmap();
  return nullBitmap;
}
...
bool inside( const IntRect& r, const Bitmap& bmp ){
  ...
  if ( r.x + r.w > static_cast<int>(bmp.m_w) || r.y + r.h > static_cast<int>(bmp.m_h) ){
    return false;
  }
  return true;
}
...
void replace_color( Bitmap& bmp, const OldColor& in_oldColor, const NewColor& in_newColor ){
  const faint::Color& oldColor(in_oldColor.Get());
  const faint::Color& newColor(in_newColor.Get());
  for ( size_t y = 0; y != bmp.m_h; y++ ){
...
faint::Bitmap rotate_90cw( const Bitmap& src ){
...
faint::Bitmap scale(const Bitmap& bmp, const Scale& scale, ScaleQuality::type quality){
  switch ( quality ){
...
faint::Bitmap scale_bilinear( const Bitmap& src, const Scale& scale ){
  IntSize newSize(constrained(truncated(floated(src.GetSize()) * abs(scale)),
      min_t(1), min_t(1)));
  if ( newSize == src.GetSize() ){
    return faint::Bitmap(src);
  }

  Bitmap dst(cairo_compatible_bitmap(newSize));
  const faint::coord x_ratio = floated(src.m_w - 1) / floated(newSize.w);
  const faint::coord y_ratio = floated(src.m_h - 1) / floated(newSize.h);
  ...
  for ( int yDst = 0; yDst != newSize.h; yDst++ ){
    for ( int xDst = 0; xDst != newSize.w; xDst++ ){
      ...
      const faint::coord x_diff = x_ratio * xDst - xSrc;
      const faint::coord y_diff = y_ratio * yDst - ySrc;
      ...
      const faint::uchar blue = static_cast<faint::uchar>((a.b)*(1-x_diff)*(1-y_diff) + (b.b)*(x_diff)*(1-y_diff) +
	(c.b)*(y_diff)*(1-x_diff) + (d.b)*(x_diff*y_diff));

      const faint::uchar green = static_cast<faint::uchar>( a.g*(1-x_diff)*(1-y_diff) + b.g*(x_diff)*(1-y_diff) +
	(c.g)*(y_diff)*(1-x_diff) + (d.g)*(x_diff*y_diff) );

      const faint::uchar red = static_cast<faint::uchar>( a.r * (1-x_diff)*(1-y_diff) + b.r*(x_diff)*(1-y_diff) +
	c.r*(y_diff)*(1-x_diff) + (d.r)*(x_diff*y_diff) );

      const faint::uchar alpha = static_cast<faint::uchar>( a.a * (1-x_diff)*(1-y_diff) + b.a*(x_diff)*(1-y_diff) +
...
faint::Bitmap scaled_subbitmap( const Bitmap& src, const Scale& scale, const IntRect& r ){
  assert(r.x >= 0 );
  assert(r.y >= 0 );
...
faint::Bitmap scale_nearest( const Bitmap& src, int scale ){
...
faint::Bitmap scale_nearest( const Bitmap& src, const Scale& scale ){
