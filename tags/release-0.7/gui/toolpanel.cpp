...
public:
  ToolPanelImpl( wxWindow* parent, SettingNotifier& notifier, StatusInterface& status, ArtContainer& art )
    : wxPanel( parent )
  {
    const int borderSize = 5;
    wxBoxSizer* outerSizer = new wxBoxSizer(wxVERTICAL);

    m_toolbar = new Toolbar( this, status, art );
    outerSizer->Add( m_toolbar, 0, wxEXPAND | wxALL, borderSize );

    m_toolSettingPanel = new ToolSettingPanel( this, notifier, status, art );
...
ToolPanel::ToolPanel( wxWindow* parent, SettingNotifier& notifier, StatusInterface& status, ArtContainer& art ){
  m_impl = new ToolPanelImpl( parent, notifier, status, art );
}
...
void ToolPanel::SelectLayer( Layer::type layer ){
