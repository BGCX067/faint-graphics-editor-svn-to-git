...
#include "generate/output/generated_methods.hh"
...
static PyObject* get_key( PyObject*, PyObject* ){
...
static PyObject* int_bind_key( PyObject*, PyObject* args ){
  ...
  if ( !PyArg_ParseTuple( args, "ii", &key, &modifiers ) ){
    PyErr_SetString(PyExc_ValueError, "Internal error: Invalid key passed to int_bind_key");
    return 0;
  }
...
static PyObject* int_unbind_key( PyObject*, PyObject* args ){
  ...
  int modifiers=0;
  if ( !PyArg_ParseTuple( args, "ii", &key, &modifiers ) ){
    PyErr_SetString(PyExc_ValueError, "Internal error: Invalid key passed to int_unbind_key");
    return 0;
  }
...
PyObject* tool( PyObject*, PyObject* args ){
  ...
  if ( !( T_MIN_ID <= toolId && toolId <= T_MAX_ID ) ){
    char error[255];
    sprintf( error, "Invalid tool identifier. Valid tool identifiers are %d to %d.", T_MIN_ID, T_MAX_ID );
...
PyObject* get_ini_path( PyObject*, PyObject* ){
  return Py_BuildValue("s", GetIniFilePath().c_str());
}

PyObject* get_ini_dir( PyObject*, PyObject* ){
  return Py_BuildValue("s", GetHomeDir().c_str());
}
...
PyObject* set_layer( PyObject*, PyObject* args){
  ...
  if ( ValidLayer(layer) ){
    GetAppContext().SetLayer( (ToLayer(layer)) );
    return Py_BuildValue("");
  }

  std::stringstream ss;
  ss << "Invalid value for layer.\nAcceptable values are:\n " << LAYER_RASTER << " - Raster layer\n " << LAYER_OBJECT << " - Object layer\n";
  PyErr_SetString( PyExc_ValueError, ss.str().c_str() );
  return 0;
}

PyObject* get_zoom( PyObject*, PyObject* ){
  CanvasInterface& canvas = GetAppContext().GetActiveCanvas();
  return Py_BuildValue("d", canvas.GetZoom()); // Fixme: Hide d
}

PyObject* zoom_default( PyObject*, PyObject* ){
  CanvasInterface& canvas = GetAppContext().GetActiveCanvas();
  canvas.ZoomDefault();
  return Py_BuildValue("");
}

PyObject* zoom_in( PyObject*, PyObject* ){
  CanvasInterface& canvas = GetAppContext().GetActiveCanvas();
  canvas.ZoomIn();
  return Py_BuildValue("");
}

PyObject* zoom_out( PyObject*, PyObject* ){
  CanvasInterface& canvas = GetAppContext().GetActiveCanvas();
  canvas.ZoomOut();
  return Py_BuildValue("");
}
...
PyObject* add_format( PyObject*, PyObject* args ){
  ...
  if ( loader == 0 && saver == 0 ){
    PyErr_SetString(PyExc_TypeError, "saver and loader both None");
    return 0;
  }
  PyFileFormat* f = new PyFileFormat( loader, saver, name, extension );
...
PyObject* list_formats( PyObject*, PyObject* ){
  ...
  for ( size_t i = 0; i != formats.size(); i++ ){
    ...
    PyObject* o = Py_BuildValue("ss", frm->GetLabel().c_str(), frm->GetFormat().c_str() );
...
PyObject* to_svg_path( PyObject*, PyObject* args ){
  ...
  if ( ellipse != 0 ){
    std::stringstream ss;
    std::vector<Point> v = EllipseAsPath( ellipse->GetTri() );
    ss << "M " << v[0].x << "," << v[0].y << " ";
...
PyObject* autosize_text( PyObject*, PyObject* args ){
  ...
  p.first->RunCommand( AutoCropText( p.second ) );
  return Py_BuildValue("");
}
...
PyObject* autosize_raster( PyObject*, PyObject* args ){
  ...
  p.first->RunCommand( AutoCropRaster( p.second ) );
  return Py_BuildValue("");
}
...
PyObject* one_color_bg( PyObject*, PyObject* args ){
  ...
  faint::Color color = GetColor( bmp, 0, 0 );
  for ( size_t y = 0; y != bmp.m_h; y++ ){
    for ( size_t x = 0; x != bmp.m_w; x++ ){
      if ( GetColor( bmp, x,y ) != color ){
        return Py_False;
      }
    }
  }
 ...
 static PyMethodDef faint_interface_methods[] = {
   {"add_format", add_format, METH_VARARGS, "add_format(format)\nAdd a file format." }, // Fixme: More information
   ...
   {"get_zoom", get_zoom, METH_NOARGS, "get_zoom() -> zoom value\n\nReturns the current zoom level."},
   ...
   {"int_get_key", get_key, METH_VARARGS, "Faint framework function, not indended for use."},
   ...
   {"list_images", get_canvas_list, METH_VARARGS, "Retrieve a list of all open documents"},
   ...
   {"zoom_default", zoom_default, METH_VARARGS, "Zoom default"},
   {"zoom_in", zoom_in, METH_VARARGS, "Zoom in"},
   {"zoom_out", zoom_out, METH_VARARGS, "Zoom out"},
   {NULL, NULL, 0, NULL}
 };

void InitializeFaintModule(){
