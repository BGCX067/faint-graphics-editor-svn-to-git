...
public:
  AutoCompleteState(){
    m_index = 0;
    m_ac.add( "Canvas" );
    m_ac.add( "get_objects()" );
    m_ac.add( "get_selected()" );
    m_ac.add( "get_selection()" );
    m_ac.add( "set_selection(" );
    m_ac.add( "get_settings()" );
    m_ac.add( "get_active_image()");
    m_ac.add( "list_images()" );
    m_ac.add( "list_fonts()" );
    m_ac.add( "print_object_types()" );
    m_ac.add( "print_commands()" );
    m_ac.add( "print" );
    m_ac.add( "flatten()" );
    m_ac.add( "Rect(" );
    m_ac.add( "Ellipse(" );
...
inline void ForwardChar( wxTextCtrl& text ){
  text.SetInsertionPoint( text.GetInsertionPoint() + 1 );
}

inline void BackwardChar( wxTextCtrl& text ){
  text.SetInsertionPoint( text.GetInsertionPoint() -1 );
}
...
bool InQuote( const wxString& text ){
  int quotes = 0;
...
/* The OnKeyDown handler is used for commands (e.g. ctrl+c etc.),
   as opposed to the OnChar handler
...
void InterpreterCtrl::OnKeyDown( wxKeyEvent& event ){
  ...
  if ( keyCode == WXK_TAB ){
    if ( InQuote( GetRange( m_inputStart, GetLastPosition() ) ) ) {
