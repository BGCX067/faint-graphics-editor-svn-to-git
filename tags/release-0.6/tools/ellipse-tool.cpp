...
bool EllipseTool::Draw( FaintDC& dc, Overlays&, const Point& ){
  if ( m_active ){
...
bool EllipseTool::DrawBeforeZoom(Layer::type layerType) const{
...
Cursor::type EllipseTool::GetCursor( const CursorPositionInfo& ) const{
...
ToolResult EllipseTool::Motion( const CursorPositionInfo& info ){
  ...
  bool antiAlias = info.layerType == Layer::OBJECT;
  m_settings.Set( ts_AntiAlias, antiAlias );
  m_p2 = info.pos;
  ...
  if ( centerPoint && constrainToCircle ) {
    ...
    info.status->SetText( str_center_radius( m_origP1, rx, ry) , 0 );
  }
