...
public:
  ...
  void Add( Object* obj, const select_added& select, const deselect_old& deselect ){
    m_frame->Add(obj);
    if ( select.Get() ){
      m_canvas.SelectObject( obj, deselect ); // Fixme: Move object selection to frames? (Yes, duh)
    }
  }
  ...
  void Add( Object* obj, size_t z, const select_added& select, const deselect_old& deselect ){
    m_frame->Add(obj, z);
    if ( select.Get() ){
      m_canvas.SelectObject( obj, deselect ); // Fixme
    }
  }
  ...
  void AddFrame( faint::Image* image, size_t index ){
  ...
  faint::Image& GetFrame(size_t index){
  ...
  void Remove( Object* obj ){
    m_canvas.DeselectObject(obj); // Fixme: Move to image
    m_frame->Remove(obj);
  }
  ...
  void ReorderFrame(size_t newIndex, size_t oldIndex){
  ...
  void SetFrameDelay( size_t num, const delay_t& delay ){
    faint::ImageList& images(m_canvas.GetImageList());
    assert(num < images.GetNumImages() );
    images.SetDelay(num, delay);
  }
...
public:
  ...
  const faint::Image& GetFrame(size_t index){
    return m_canvas->GetImageList().GetImage(index);
  }

  int GetFrameDelay( size_t num ) const {
    const faint::ImageList& images(m_canvas->GetImageList());
    assert(num < images.GetNumImages() );
    return images.GetDelay(num);
  }
  ...
  void SelectFrame( size_t frame ){
    m_canvas->SelectFrame(frame);
  }
...
void CanvasScroller::AddFrame( faint::Image* image, size_t index ){
...
void CanvasScroller::BundleUndo( size_t count, const std::string& name ){
  ...
  m_undoList.push_back( std::make_pair(new CommandBunch(get_command_type(cmds),
    cmds,
    bunch_name(name)), bundled.front().second));
...
bool CanvasScroller::CanUndo() const{
  // Fixme: Also allow undo while a tool is active (to cancel the tool, e.g. polygon)
...
objects_t& CanvasScroller::GetObjects(){
...
size_t CanvasScroller::GetSelectedFrame() const{
...
bool CanvasScroller::Has( const ObjectId& id ){
  objects_t& objects = m_images.Active().GetObjects();
  for ( size_t i = 0; i != objects.size(); i++ ){
...
void CanvasScroller::OnPaint( wxPaintEvent& ){
  ...
  dc.SetScale( zoom );
  objects_t& objects = active.GetObjects();
  for ( size_t i = 0; i != objects.size(); i++ ){
...
void CanvasScroller::PasteBitmap( const faint::Bitmap& bmp ){
  ...
  if ( oldSelection.Floating() ){
    RunCommand( oldSelection.StampFloatingSelection() ); // Fixme: Bundle with paste
  }
  Settings settings(GetAppContext().GetToolSettings());
  RunCommand( get_paste_raster_bitmap_command( bmp,
...
void CanvasScroller::PasteBitmapAsObject( const faint::Bitmap& bmp ){
  ...
  RunCommand( get_add_object_command( rasterObj, select_added(false) ) ); // Fixme: name
...
void CanvasScroller::PreviousFrame(){
  const size_t maxIndex = m_images.GetNumImages();
  const size_t oldIndex = m_images.GetSelectedIndex();
  const size_t index( oldIndex == 0 ? maxIndex - 1 : oldIndex - 1 );
  SelectFrame(index);
...
void CanvasScroller::NextFrame(){
  const size_t maxIndex = m_images.GetNumImages();
  const size_t oldIndex = m_images.GetSelectedIndex();
  const size_t index((oldIndex + 1) % maxIndex);
  SelectFrame(index);
  GetAppContext().OnDocumentStateChange(m_canvasId); // Fixme: For animation control
}
...
void CanvasScroller::Redo(){
  ...
  if ( tool->CanRedo() ){
    ...
    Refresh(); // Fixme: For MoveContent which refreshes only the bitmap area, leaves droppings when rotated <- Fixme: Still required?
    return;
  }
  ...
  RunCommand( redone.first, false, redone.second );
}
...
void CanvasScroller::RemoveFrame( size_t index ){
  m_images.Remove(index);
  GetAppContext().OnDocumentStateChange(m_canvasId); // Fixme: Queue!
}

void CanvasScroller::ReorderFrame( size_t newIndex, size_t oldIndex ){
...
void CanvasScroller::SelectObjects( const objects_t& objects, const deselect_old& deselectOld ){
  ...
  objects_t imageObjects(active.GetObjects()); // Fixme: Why copy?
  objects_t objectSelection(active.GetObjectSelection());
  for ( size_t i = 0; i != objects.size(); i++ ){
...
void CanvasScroller::SelectFrame( size_t index ){
  assert( index < m_images.GetNumImages() );
  if ( index != m_images.GetSelectedIndex() ){
    Preempt(PreemptOption::ALLOW_COMMAND);
    m_images.SetSelectedIndex( index );
    m_cmdContext->Reset();
...
RectangleSelectTool* CanvasScroller::SwitchToRasterSelection(){ // Fixme: Consider removing
  GetAppContext().SelectTool( T_RECT_SEL );
  RectangleSelectTool* selectionTool = dynamic_cast<RectangleSelectTool*>(m_toolWrap.GetTool());
...
void CanvasScroller::Undo(){
  if ( m_toolWrap.GetTool()->CanUndo() ){
    ...
    MousePosRefresh();
    GetAppContext().OnDocumentStateChange(m_canvasId); // Fixme: For undo/redo-state
    Refresh(); // Fixme: For MoveContent which refreshes only the bitmap area, leaves droppings when rotated <- Fixed?
    return;
  }
...
void CanvasScroller::RunCommand( Command* cmd, bool clearRedo, faint::Image* activeImage ){
  if ( activeImage == 0 ){
