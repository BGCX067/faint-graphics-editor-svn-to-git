...
public:
  ...
  void ShowPythonConsole(){
    if ( m_interpreterFrame->IsHidden() ){
      ...
      if ( m_interpreterFrame->IsIconized() ){
...
void MainFrame::OnClose( wxCloseEvent& event ){
  if ( m_tabControl->UnsavedDocuments() && event.CanVeto() ){
    bool quit = ask_exit_unsaved_changes(this); // Fixme: Lazy! Use warning and add labeled button ("Exit")
    if ( !quit ){
      event.Veto();
      return;
    }
  ...
  try {
    ...
    for ( unsigned int i = 0; i!= filePaths.size(); i++ ){
      ...
      if ( wxFileName(filePath).FileExists(filePath) ){
        ...
        show_file_not_found_error( this, filePath );
      }
    }
  }
