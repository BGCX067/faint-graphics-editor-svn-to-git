...
namespace faint {
  ...
  BITMAPRETURN rotate( const faint::Bitmap& bmp, faint::radian angle, const faint::Color& bgColor ) {
    ...
    faint::Bitmap bmpDst( faint::cairo_compatible_bitmap(IntSize((int) ceil(max4(x1, x2, x3, x4)) - OfX,
	  (int) ceil(max4(y1, y2, y3, y4)) - OfY ) ));
    clear( bmpDst, bgColor );
    faint::uchar* dstRow = bmpDst.m_data;

    const faint::uchar* src = bmp.m_data;
    double divisor = cA*cA + sA*sA;

    const int wSrc( static_cast<int>( bmp.m_w ) );
    const int hSrc( static_cast<int>( bmp.m_h ) );
    const int wDst( static_cast<int>( bmpDst.m_w ) );
    const int hDst( static_cast<int>( bmpDst.m_h ) );

    // Iterate over the destination bitmap
    for ( int stepY = 0; stepY != hDst; stepY++ ) {
      for ( int stepX = 0; stepX != wDst; stepX++ ) {
        ...
        if ( orgX >= 0 && orgY >= 0 && orgX < wSrc && orgY < hSrc ) {
