...
bool Clipboard::GetText( faint::utf8_string& dest ){
  ...
  wxString wxStr( textObject.GetText() );
...
void Clipboard::SetBitmap( const faint::Bitmap& bmp, const faint::Color& bgCol ){
  assert(m_ok);
  wxDataObjectComposite* composite = new wxDataObjectComposite;
  composite->Add( new faint::FaintBitmapDataObject( bmp ), true );
  composite->Add( new wxBitmapDataObject( to_wx( faint::alpha_blended( bmp, bgCol ) ) ) );
