...
public:
  ...
  RasterSelectionState()
    : copy(false),
      floating(false),
      options(false, faint::Color(255,255,255), false)
  {}
  // A floating pasted bitmap
  RasterSelectionState(const faint::Bitmap&, const IntPoint&, const RasterSelectionOptions&);

  // A non-floating rectangle selection
  RasterSelectionState(const IntRect&, const RasterSelectionOptions&);

  // Non-pasted float (leaves a hole)
  RasterSelectionState(const faint::Bitmap&, const IntPoint&, const IntRect& oldRect, const RasterSelectionOptions&);
private:
  RasterSelectionState& operator=(const RasterSelectionState&);
  friend class RasterSelection;
  bool copy;
  bool floating;
  RasterSelectionOptions options;
  IntRect rect;
  IntRect oldRect;
