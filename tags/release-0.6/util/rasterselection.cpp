...
#include "commands/cmdmovecontent.hh"
#include "geo/tri.hh"
#include "rasterselection.hh"
...
RasterSelectionState::RasterSelectionState(const faint::Bitmap& bmp, const IntPoint& topLeft, const RasterSelectionOptions& in_options )
  : copy(true),
    floating(true),
    floatingBmp(bmp),
    oldRect(IntRect()),
...
RasterSelectionState::RasterSelectionState(const IntRect& r, const RasterSelectionOptions& in_options )
  : copy(false),
    floating(false),
    oldRect(IntRect()),
    options(in_options),
...
RasterSelectionState::RasterSelectionState( const faint::Bitmap& bmp, const IntPoint& topLeft, const IntRect& in_oldRect, const RasterSelectionOptions& in_options )
  : copy(false),
    floating(true),
    floatingBmp(bmp),
    oldRect(in_oldRect),
...
void RasterSelection::DrawFloating(FaintDC& dc) const{
  ...
  if ( !m_state.copy ){
    ...
    dc.Rectangle( tri_from_rect(floated(m_state.oldRect)),
      eraser_rectangle_settings(m_state.options.bgCol));
  }

  dc.Bitmap( m_state.floatingBmp, floated(m_state.rect.TopLeft()), bitmap_mask_settings(m_state.options.mask, m_state.options.bgCol, m_state.options.alpha) );
}
...
Command* RasterSelection::StampFloatingSelection() const{
  ...
  return  new CmdMoveContent( m_state.floatingBmp,
    ...
    bitmap_mask_settings(m_state.options.mask, m_state.options.bgCol, m_state.options.alpha),
