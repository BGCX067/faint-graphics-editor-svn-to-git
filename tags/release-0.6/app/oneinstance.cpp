...
public:
  bool OnExec( const wxString& /*topic*/, const wxString& filename ){
    GetAppContext().Load( std::string(filename), change_tab(false) );
    return true;
  }
};
...
bool send_files_to_server( const std::vector<std::string>& files, const std::string& port ){
  ...
  while ( attempt < 3 && connection == 0 ){
    connection = client->MakeConnection( hostname, port, g_faintTopicFiles);
    attempt++;
    if ( connection == 0 ){
      wxThread::Sleep( 500 );
    }
  ...
  if ( connection == 0 ){
    // Connecting failed to the old instance failed.
    delete client;
    return false;
  }
