...
bool TextIdle::Draw( FaintDC&, Overlays&, const Point& ){
  return false;
}

bool TextIdle::DrawBeforeZoom( Layer::type layer ) const{
...
Cursor::type TextIdle::GetCursor( const CursorPositionInfo& info ) const{
  return (hovered_selected_text(info) == 0) ?
    Cursor::TEXT_CROSSHAIR :
    Cursor::CARET;
}
