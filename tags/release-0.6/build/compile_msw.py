...
def get_cl_switches(rvalue_ref, debug):
...
def compile( file, faintRoot, includes, out, err, objRoot, wxRoot, strip, rvalue_ref, version, debug ):
    file = file.replace("/", "\\")
    ...
    switches = get_cl_switches(rvalue_ref, debug)
...
def link( files, faintRoot, wxRoot, cairo_lib, python_lib, pango_lib, glib_lib, debug ):
    ...
    if debug:
        flags += " /OUT:Faintd.exe /DEBUG /PDB:Faint.pdb"
    else:
        flags += " /OUT:Faint.exe"
