...
def parsePolyLineNode( node ):
    ...
    if arrowhead:
        ...
        vals[-2], vals[-1] = actual_line_end(vals[-2], vals[-1], angle, s.linewidth)
    return ["polyline",
            vals,
            s,
            parseNodeTransform(node)]
...
def _parsePathPath( node ):
    if not node.hasAttribute('d'):
        raise Exception("Somewhere in the SVG, a path node is missing its definition ('d').\nThis is probably fine, but Faint can't handle it.")
