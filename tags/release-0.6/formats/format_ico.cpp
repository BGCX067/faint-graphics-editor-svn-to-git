...
void FormatICO::Load( const std::string& filename, ImageProps& imageProps ){
  faint::load_icon( filename, imageProps );
}

SaveResult FormatICO::Save( const std::string& filename, CanvasInterface& canvas ){
  std::vector<const faint::Bitmap> bitmaps;
  for ( size_t i = 0; i != canvas.GetNumFrames(); i++ ){
    bitmaps.push_back(faint::flatten(canvas.GetFrame(i)));
  }
