...
public:
  RasterSelectionMove(const IntPoint& offset, const IntPoint& topLeft, const copy_selected& copy, bool float_selected, Settings& s, Canvas& canvas)
    : m_canvas(canvas),
  ...
  TaskResult MouseUp(const PosInfo& info) override{
    m_command.Set(set_raster_selection_command(New(m_selection->GetState()),
      ...
      m_selection->Floating() ? "Move Selected Content" : "Move Selection", true));
...
Task* raster_selection_move_task(const IntPoint& offset, const IntPoint& topLeft, const copy_selected& copySelected, bool floatSelected, Settings& settings, Canvas& canvas)
{
  return new RasterSelectionMove(offset, topLeft,
    copySelected, floatSelected,
    settings, canvas);
}
