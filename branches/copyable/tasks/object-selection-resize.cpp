...
class ResizeObjectBase : public Task {
public:
  ...
  void Draw(FaintDC&, Overlays& overlays, const Point&) final override{
  ...
  IntRect GetRefreshRect(const IntRect&, const Point&) const final override{
    return inflated(m_object->GetRefreshRect(), objectHandleWidth);
  }
  ...
  TaskResult MouseMove(const PosInfo& info) override{
    ...
    if (info.modifiers.Primary()){
      ...
      if (m_lockY){
        ...
        p.x = snap_x(p.x, objects, info.canvas.GetGrid(), oldRect.Top(), oldRect.Bottom());
      }
      else if (m_lockX){
        Rect oldRect(bounding_rect(m_oldTri));
        p.y = snap_y(p.y, objects, info.canvas.GetGrid(), oldRect.Left(), oldRect.Right());
      }
      else {
    ...
    Scale scale((m_lockX || dx == 0)? 1.0 : (p.x - m_origin.x) / dx,
      (m_lockY || dy == 0) ? 1.0 : (p.y - m_origin.y) / dy);

    bool constrain = info.modifiers.Secondary();
...
Task* resize_object_task(Object* obj, Handle handle){
