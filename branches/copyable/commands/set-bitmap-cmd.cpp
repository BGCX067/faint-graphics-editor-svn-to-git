...
public:
  SetObjectBitmapCommand(ObjRaster* object, const Bitmap& bmp, const Tri& tri, const utf8_string& name)
    : Command(CommandType::OBJECT),
      m_object(object),
      m_bitmap(copy(bmp)),
      m_oldBitmap(copy(object->GetBitmap())),
      m_tri(tri),
...
public:
  SetBitmapCommand(const Bitmap& bmp, const IntPoint& topLeft, const utf8_string& name)
...
Command* set_object_bitmap_command(ObjRaster* obj, const Bitmap& bmp, const Tri& tri, const utf8_string& name){
  return new SetObjectBitmapCommand(obj, bmp, tri, name);
}

Command* set_bitmap_command(const Bitmap& bmp, const IntPoint& topLeft, const utf8_string& name){
