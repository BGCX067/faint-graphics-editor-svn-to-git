...
class FunctionCommand : public BitmapCommand {
 public:
  FunctionCommand(const utf8_string& name, const std::function<void(Bitmap&)>& func)
    : m_name(name),
      m_func(func)
  {}
...
BitmapCommand* get_function_command(const utf8_string& name, const std::function<void(Bitmap&)>& func){
