...
public:
  FormatGIF()
    : Format(FileExtension("gif"), label_t(utf8_string("Graphics Interchange Format (GIF)")), can_save(true), can_load(true))
  {}

  void Load(const FilePath& filePath, ImageProps& imageProps) override{
    load_gif(filePath, imageProps);
  }
  SaveResult Save(const FilePath& filePath, Canvas& canvas) override{
    ...
    const int numFrames(canvas.GetNumFrames().Get());
    for (int i = 0; i != numFrames; i++){
      Bitmap bmp(flatten(canvas.GetFrame(index_t(i))));
