...
public:
  LoadGifError( const faint::utf8_string& error )
    : m_error(error)
  {}
...
class Alphabet{
public:
  Alphabet(int in_codeSize)
    : codeSize(in_codeSize),
...
public:
  ...
  faint::Color Get(int index) const{
    assert(0 <= index);
    assert(index < m_entries);
    return faint::Color(static_cast<unsigned char>(m_data[index * 3]),
...
Disposal to_disposal(std::underlying_type<Disposal>::type value){
  typedef decltype(value) T;
  if ( value < static_cast<T>(Disposal::MIN_VALUE) || static_cast<T>(Disposal::MAX_VALUE) < value ){
    return Disposal::NOT_SPECIFIED;
  }
  return static_cast<Disposal>(value);
}
...
public:
  ...
  DisposeInfo(Disposal in_disposal, const IntRect& in_rect)
    : disposal(in_disposal),
      rect(in_rect)
  {}

  void Set(Disposal in_disposal, const IntRect& in_rect){
    disposal = in_disposal;
    rect = in_rect;
  }
...
public:
  ...
  delay_t GetDelay(){
    return delay_t(delayTime_cs * 10);
  }
...
static GifVer read_gif_identifier(BinaryReader& f){
  char data[6];
  f.read(data, 6);
  if ( !f.good() ){
...
static LogicalScreenDescriptor read_logical_screen_descriptor(faint::BinaryReader& f ){
...
static void read_color_table(faint::BinaryReader& f, ColorTable& table){
  f.read(table.RawData(), table.RawLength());
  if ( !f.good() ){
    throw LoadGifError("Premature EOF in color table");
  }
}

static int stream_getc_i(BinaryReader& f){
  char c;
  f.read(&c, 1);
  if ( !f.good() ){
    throw LoadGifError("Premature EOF");
...
static int get_code(BinaryReader& f, const Alphabet& a, ParseData& pd ){
  ...
  while ( a.bits > pd.restBits ){
    if ( pd.restByte == 0 ){
      ...
      pd.restByte = static_cast<unsigned int>(stream_getc_i(f));
      ...
      f.read(pd.buffer, resigned(pd.restByte));
      if ( !f.good() ){
...
static int flush_stack( Alphabet& a, IndexBuffer& img, int& x, int& y, int& pass, int code ){
  while ( !a.Empty() ){
...
static void read_image_data( BinaryReader& f, IndexBuffer& img ){
...
static ImageDescriptor read_image_descriptor( BinaryReader& f ){
  char buf[9];
  f.read(buf, 9);
  if ( !f.good() ){
...
static GraphicControlExtension read_graphic_control_extension(BinaryReader& f){
  char data[6];
  f.read(data, 6);
  if ( !f.good() ){
...
static void read_application_extension( BinaryReader& f ){
  ...
  static void read_away_unknown_extension( BinaryReader& f, unsigned char type ){
  char entry;
  std::streampos extensionStart = f.tellg();
  do {
    f.read(&entry, 1);
    if ( !f.good() ){
      std::stringstream ss;
      ss << "Premature EOF within extension block of type: " << std::hex << static_cast<int>(type) << std::endl << "Starting at " << extensionStart;
      throw LoadGifError(ss.str().c_str());
    }
  } while ( entry != 0 );
}

static faint::Bitmap get_bg(ImageProps& props, const LogicalScreenDescriptor& lsd, const ColorTable& /*colorTable*/, const DisposeInfo& disposeInfo ){
  if ( props.GetNumFrames() == 0 ){
    return faint::Bitmap(lsd.size, faint::color_transparent_white());
  }
  ...
  case Disposal::RESTORE_TO_BACKGROUND_COLOR:
    {
      Bitmap bg(copy(props.GetFrame(props.GetNumFrames() - 1).GetBitmap()));
      fill_rect_color(bg, disposeInfo.rect, faint::color_transparent_white());
      return bg;
    }
  case Disposal::RESTORE_TO_PREVIOUS:
    if (props.GetNumFrames() > 1){
      return copy(props.GetFrame(props.GetNumFrames() - 2).GetBitmap()); // Fixme: unnecessary copy
    }
    else{
      return Bitmap(lsd.size, faint::color_transparent_white());
    }
  ...
  default:
    // Fixme: Nasty copy
    return copy(props.GetFrame(props.GetNumFrames() - 1).GetBitmap());
  }
}

static bool read_block(BinaryReader& f, ImageProps& props, LogicalScreenDescriptor& lsd, ColorTable& globalColorTable, GraphicControlExtension& gce, DisposeInfo& disposeInfo ){
  char identifier;
  f.read(&identifier, 1);
  if ( !f.good() ){
    return false;
  ...
  if ( identifier == 0x21 ){
    ...
    if ( (unsigned char)identifier == 0xf9 ){
      gce = read_graphic_control_extension(f);
    }
    else if ( (unsigned char)identifier == 0xff ){
      read_application_extension(f);
    }
    else{
      read_away_unknown_extension(f, (unsigned char)identifier);
    }
  }
  else if ( identifier == 0x2c ){
    ImageDescriptor descr = read_image_descriptor(f);
    ...
    if ( descr.localColorTable ){
      read_color_table(f, localColorTable);
    }
    ...
    read_image_data(f, img);

    faint::Bitmap bmp(descr.size);
    ...
    faint::Bitmap bg = get_bg(props, lsd, colorTable, disposeInfo);
    faint::blend(bmp, onto(bg), descr.offset);
...
void load_gif(const FilePath& filePath, ImageProps& props ){
  try{
    faint::BinaryReader f(filePath);
    if ( !f.good() ){
    ...
    if ( props.GetNumFrames() > 1 && gifVersion == GifVer::GIF87a ){
      props.AddWarning(faint::utf8_string("GIF specified as GIF87a, but contains multiple frames."));
    }
  }
