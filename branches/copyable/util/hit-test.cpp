...
#include "geo/line.hh"
#include "geo/size.hh"
#include "gui/gui-constants.hh" // Fixme
...
namespace faint{
static std::pair<Object*, Hit> object_at(const Point& p, const Image& image, const CanvasGeo& geo, Bitmap& hitTestBuffer){
...
template<typename T>
static int point_hit_test(const T& points, const IntPoint& ptView, const CanvasGeo& g){
  const Point imagePos(ptView.x - g.border.w + g.pos.x, ptView.y - g.border.h + g.pos.y);
...
template<Handle H1, Handle H2, typename T>
Optional<Handle> find_nearest(const Point& pos, const T& points, coord zoom, coord& minDistance){
...
template<typename T>
static Optional<Handle> get_side(const T& points, const IntPoint& ptView, const CanvasGeo& g){
  ...
  if (between<Handle::P0P1, Handle::P2P3, T>(pos, points, zoom)){
    nearest |= find_nearest<Handle::P0P2, Handle::P1P3>(pos, points, zoom, minDistance);
  }
  if (between<Handle::P0P2, Handle::P1P3, T>(pos, points, zoom)){
    nearest |= find_nearest<Handle::P0P1, Handle::P2P3>(pos, points, zoom, minDistance);
  }
  return nearest;
}

template<typename T>
static Optional<Handle> get_handle(const T& points, const IntPoint& viewPos, const CanvasGeo& g){
...
ObjectInfo hit_test(const IntPoint& ptView, const Image& image, const CanvasGeo& geo){
  ...
  for (Object* objTemp : objectSelection){
    if (point_edit_enabled(objTemp)){
      ...
      if (handleIndex != - 1){
        handleInfo.Set(EitherHandle(std::make_pair(handleIndex, HandleType::MOVABLE_POINT)));
      ...
      handleIndex = point_hit_test(objTemp->GetExtensionPoints(), ptView, geo);
      if (handleIndex != -1){
        handleInfo.Set(EitherHandle(std::make_pair(handleIndex, HandleType::EXTENSION_POINT)));
        hitStatus = Hit::BOUNDARY;
        selected = true;
        object = objTemp;
        break;
    ...
    if (resize_handles_enabled(objTemp)){
      ...
      if (auto handle = get_handle(cornerPoints, ptView, geo)){
  ...
  if (handleInfo.NotSet()){
    ...
    std::pair<Object*, Hit> hitObject = object_at(ptImg, image, geo, hitTestBuffer);
