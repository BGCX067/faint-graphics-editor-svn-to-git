...
auto visit(const RasterSelection& s,
  ...
  const COPYING_FUNC& copying) -> decltype(empty(Empty()))
{
  if (s.Empty()){
    return empty(Empty());
  }
    ...
    return moving(Moving(s.GetRect(), s.GetOldRect(), s.GetBitmap(), s.GetOptions()));
  }
  else{
...
auto visit(const RasterSelection& s,
  const EMPTY_FUNC& empty,
  const RECT_FUNC& rect,
  const FLOATING_FUNC& floating) -> decltype(empty(Empty()))
{
  if (s.Empty()){
    return empty(Empty());
  }
...
auto visit(const Existing& e,
  const RECT_FUNC& rect,
  const MOVING_FUNC& moving,
  const COPYING_FUNC& copying) -> decltype(rect(Rectangle(e.Get().GetRect(), e.Get().GetOptions())))
{
