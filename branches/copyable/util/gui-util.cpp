...
namespace faint{
  ...
  wxButton* noiseless_button(wxWindow* parent, const wxBitmap& bmp, const tooltip_t& tooltip, const wxSize& size){
    wxButton* button = noiseless_button(parent, "", tooltip, size);
    button->SetBitmap(bmp);
    return button;
  }

  wxButton* noiseless_button(wxWindow* parent, const wxString& label, const tooltip_t& tooltip, const wxSize& size){
    // wxWANTS_CHARS prevents noise on keypress when button has focus
    wxButton* button = new wxButton(parent, wxID_ANY, label, wxDefaultPosition, size, wxWANTS_CHARS);
...
SaveChoice ask_close_unsaved_tab(wxWindow* parent, const Optional<FilePath>& filePath){
...
static void show_warning(wxWindow* parent, const Title& title, const wxString& message){
  wxMessageDialog dlg(parent,
    message,
    to_wx(title.Get()),
    wxOK|wxICON_WARNING);
...
void show_load_failed_error(wxWindow* parent, const FilePath& file, const utf8_string& message){
...
FileList show_open_file_dialog(wxWindow& parent, const Title& title, const Optional<DirPath>& initialPath, const wxString& filter){
...
AcceleratorEntry::AcceleratorEntry(const KeyPress& in_key, int in_cmd)
  : cmd(in_cmd),
    keyPress(in_key)
{}

LambdaAcceleratorEntry::LambdaAcceleratorEntry(const KeyPress& key, const std::function<void()>& func)
  : func(func),
    keyPress(key)
{}

wxAcceleratorTable accelerators(const std::vector<AcceleratorEntry>& entries){
  std::vector<wxAcceleratorEntry> wx_entries;
  for (const AcceleratorEntry& entry : entries){
    const KeyPress& keyPress = entry.keyPress;
    wx_entries.push_back(wxAcceleratorEntry(accel_from_key(keyPress), keyPress.GetKeyCode(), entry.cmd));
  }

  return wxAcceleratorTable(resigned(wx_entries.size()), wx_entries.data());
}

  void set_accelerators(wxWindow* window, const std::vector<LambdaAcceleratorEntry>& entries){
  std::vector<wxAcceleratorEntry> wx_entries;

  for (const auto& entry : enumerate(entries)){
    const KeyPress& keyPress = entry.item.keyPress;
    const int cmdId = entry.num;
    wx_entries.push_back(wxAcceleratorEntry(accel_from_key(keyPress), keyPress.GetKeyCode(), entry.num));
    window->Bind(wxEVT_COMMAND_MENU_SELECTED,
      [=](wxCommandEvent&){
        entries[to_size_t(cmdId)].func();
      }, entry.num);
  }
