...
Optional<Bitmap> Clipboard::GetBitmap(){
  ...
  if (wxTheClipboard->GetData(composite)){
    ...
    if (format == wxDataFormat("FaintBitmap")){
      return Optional<Bitmap>(std::move(copy(bmpObject_faint->GetBitmap())));
    }
    else{
      wxBitmap wxBmp = bmpObject_wx->GetBitmap();
      return Optional<Bitmap>(to_faint(clean_bitmap(wxBmp)));
    }
  }
  return no_option();
}
...
void Clipboard::SetBitmap(const Bitmap& bmp){
  assert(m_ok);
  SetBitmap(bmp, rgb_black());
}
