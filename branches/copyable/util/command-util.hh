...
BitmapCommand* get_aa_line_command(const IntPoint&, const IntPoint&, const ColRGB&);
bool affects_raster(const Command*);
Command* get_add_objects_command(const objects_t&, const select_added&, const utf8_string& name="Add");
...
Command* get_crop_to_selection_command(const RasterSelection&,
  const Paint& bg);
Command* get_delete_objects_command(const objects_t&, const Image&, const utf8_string& name="Delete");
...
BitmapCommand* get_boundary_fill_command(const IntPoint&, const Paint& fill, const Color& boundary);
...
Command* get_paste_raster_bitmap_command(const Bitmap&, const IntPoint&, const RasterSelection& oldSelection, const Settings&);
Command* get_objects_to_paths_command(const objects_t&, const Image&, const select_added&);

BitmapCommand* get_pinch_whirl_command(coord, const Rotation&);
...
typedef Order<Object*>::Old OldObject;
...
Command* get_resize_command(const Optional<Bitmap>&, const IntRect& newRect, const Paint& bg);
...
Command* get_scale_rotate_command(const objects_t&, const Scale&, const Angle&, const Point& origin);
...
Command* get_rotate_selection_command(const Image&, const Angle& angle, const Paint& bg);
Command* get_offset_selection_command(const Image&, const IntPoint& delta);
Command* get_scale_raster_selection_command(const Image&, const IntSize& newSize);
