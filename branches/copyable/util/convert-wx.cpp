...
#include "app/app.hh" // For get_art_container
...
const wxCursor& to_wx_cursor(Cursor cursor){
  return get_art_container().Get(cursor);
}
...
wxImage to_wx_image(const Bitmap& bmp){
  ...
  uchar* rgbData = (uchar*)malloc(to_size_t(bmp.m_w * bmp.m_h * 3)); // Using malloc to match wxWidgets free
  uchar* aData = (uchar*)malloc(to_size_t(bmp.m_w * bmp.m_h)); // Using malloc to match wxWidgets free
...
Bitmap to_faint(wxBitmap wxBmp){
  ...
  if (wxBmp.GetDepth() == 24){
    ...
    for (int y = 0; y != bmp.m_h; y++){
      ...
      for (int x = 0; x != bmp.m_w; x++){
        size_t pos = to_size_t(y * stride + x * 4);
        data[ pos + 3 ] = 255;
        data[ pos + 2 ] = p.Red();
        data[ pos + 1 ] = p.Green();
        data[ pos + 0 ] = p.Blue();
...
std::wstring iostream_friendly(const wxString& path){
  return path.ToStdWstring(); // Fixme: Verify that this means UCS2 on MSW
}
#else
