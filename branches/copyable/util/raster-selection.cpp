...
SelectionOptions::SelectionOptions(bool in_mask, const Paint& in_bg, bool in_alpha)
  : alpha(in_alpha),
    bg(in_bg),
    mask(in_mask)
{}
...
SelectionState::SelectionState(const Bitmap& bmp, const IntPoint& topLeft, const IntRect& in_oldRect)
  : copy(false),
    floating(true),
    floatingBmp(faint::copy(bmp)),
    oldRect(in_oldRect),
...
void RasterSelection::BeginFloat(const Bitmap& src, const copy_selected& copy){
  ...
  assert(inside(m_state.rect, src));
...
void RasterSelection::DrawFloating(FaintDC& dc) const{
  ...
  dc.Blit(m_state.floatingBmp, floated(m_state.rect.TopLeft()), bitmap_mask_settings(m_options.mask, m_options.bg, m_options.alpha));
}
...
void RasterSelection::SetFloatingBitmap(const Bitmap& bmp, const IntPoint& topLeft){
...
Floating::Floating(const IntRect& rect, const Bitmap& bmp, const SelectionOptions& options)
...
Moving::Moving(const IntRect& r, const IntRect& old, const Bitmap& bmp, const SelectionOptions& options)
...
Copying::Copying(const IntRect& r, const Bitmap& bmp, const SelectionOptions& options)
  : Floating(r, bmp, options)
{}
