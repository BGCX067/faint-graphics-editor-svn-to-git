...
bool can_move_backward(const Image& image, const objects_t& objects){
  ...
  // Fixme: This is a pretty nasty way to check
...
static void flatten_onto(Bitmap& bmp, const objects_t& objects, const RasterSelection& selection){
  FaintDC dc(bmp);
  selection.DrawFloating(dc);
...
Bitmap flatten(const Image& image){
  ...
  return image.GetBg().Visit(
    [&](const Bitmap& bmp){
      Bitmap copy(faint::copy(bmp));
      flatten_onto(copy, objects, selection);
      return copy;
    },
    [&](const ColorSpan& span){
      Bitmap bmp(span.size, span.color);
      flatten_onto(bmp, objects, selection);
      return bmp;
    });
}
...
void remove_missing_objects_from(objects_t& objects, const Image& image){
  auto object_not_in_image = [&image](const Object* obj){ return !image.Has(obj); };
  objects.erase(remove_if(begin(objects), end(objects), object_not_in_image), end(objects));
}
