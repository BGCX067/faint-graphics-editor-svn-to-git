...
CanvasResizeHandle::CanvasResizeHandle( HandlePos handlePos, const IntSize& canvasSize, const CanvasGeo& geo )
...
IntRect CanvasResizeHandle::GetRect() const{
  ...
  if ( m_handlePos == HandlePos::TOP_LEFT ){
    ...
    const int ch_w = g_canvasHandleSz.w;
    return IntRect(IntPoint(m_geo.border.w + width / 2 - m_geo.pos.x - ch_w / 2, bottom),
      g_canvasHandleSz);
  }
  else if ( m_handlePos == HandlePos::RIGHT_SIDE ){
    const int ch_h = g_canvasHandleSz.h;
    return IntRect(IntPoint( right, m_geo.border.h + height / 2 - m_geo.pos.y - ch_h / 2), g_canvasHandleSz);
  }
  else if ( m_handlePos == HandlePos::LEFT_SIDE ){

    const int ch_h = g_canvasHandleSz.h;
    return IntRect(IntPoint(left,m_geo.border.h + height / 2 - m_geo.pos.y - ch_h / 2),
        g_canvasHandleSz);
  }
  else {
    assert( false );
...
Optional<CanvasResizeHandle> canvas_handle_hit_test( const IntPoint& mousePos, const IntSize& canvasSize, const CanvasGeo& geo){
