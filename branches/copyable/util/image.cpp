...
static size_t get_sorted_insertion_pos(Object* obj, const objects_t& v_trg, const objects_t& v_src){
  size_t i_src = 0;
  size_t i_trg = 0;
...
Image::Image(FrameProps&& props)
  : m_bg(ColorSpan(Color(255,255,255), IntSize(0,0))),
    m_createdBitmap(false),
    m_delay(0),
    m_objects(std::move(props.GetObjects())),
    m_original(),
    m_originalObjects(m_objects),
    m_size(props.GetSize())
{
  if (props.HasBitmap()){
    m_bg.Set(std::move(props.GetBitmap()));
    m_delay = props.GetDelay();
    m_hotSpot = props.GetHotSpot();
  }
  else {
    m_size = props.GetSize();
    m_bg.Set(ColorSpan(props.GetBackgroundColor(), m_size));
  }
...
Image::Image(const Image& other)
  : m_bg(other.m_bg.Visit(
                          [](const Bitmap& bmp){
                            return Either<Bitmap, ColorSpan>(copy(bmp));
                          },
                          [](const ColorSpan& colorSpan){
                            return Either<Bitmap, ColorSpan>(colorSpan);
                          })),
...
void Image::StoreAsOriginal(){
  auto& bmp = m_bg.Get<Bitmap>();
  if (!m_createdBitmap && bmp.IsSet()){
    m_original.Set(copy(bmp.Get()));
  }
}

void Image::SetBitmap(const Bitmap& bmp){
  m_bg.Set(std::move(copy(bmp)));
}
...
IntSize Image::GetSize() const{
  const auto& bmp = m_bg.Get<Bitmap>();
  return bmp.IsSet() ?
    bmp.Get().GetSize() :
    m_size;
}
...
void Image::Revert(){
  if (!m_original.IsSet()){
    // Fixme: Use an either for original too
    m_bg.Set(ColorSpan(Color(255,255,255), m_size));
  }
  else{
    m_bg.Set(std::move(copy(m_original.Get())));
  }
}

const Either<Bitmap, ColorSpan>& Image::GetBg() const{
  return m_bg;
}

Either<Bitmap, ColorSpan>& Image::GetBg(){
  return m_bg;
}
