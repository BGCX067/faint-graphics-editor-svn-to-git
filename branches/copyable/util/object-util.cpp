...
Object* clone_as_path(Object* object){
  ...
  Points pts(object->GetPath());
  return create_path_object(pts, s);
}

Color color_at(ObjRaster* obj, const Point& imagePos){
  Bitmap bmp(IntSize(1,1), get_color_default(obj->GetSettings().Get(ts_Bg), color_white()));
  FaintDC dc(bmp);
...
utf8_string get_collective_name(const objects_t& objects){
  assert(!objects.empty());
...
bool remove(const Object* obj, const from_t<objects_t>& from){
  ...
  objects_t::iterator newEnd = std::remove_if(begin(objects), end(objects), obj_eq);
...
static bool remove_objects_from(objects_t& objects, const objects_t& remove){
  ...
  objects_t::iterator newEnd = std::remove_if(begin(objects), end(objects), should_remove);
  if (newEnd == end(objects)){
    return false;
  }
  objects.erase(newEnd, end(objects));
...
Point snap(const Point& sourcePt, const objects_t& objects, const Grid& grid, coord maxSnapDistance){
  std::vector<Point> noExtraPoints;
  return snap(sourcePt, objects, grid, noExtraPoints, maxSnapDistance);
}

Point snap(const Point& sourcePt, const objects_t& objects, const Grid& grid, const std::vector<Point>& extraPoints, coord maxSnapDistance){
...
coord snap_x(coord sourceX, const objects_t& objects, const Grid& grid, coord y0, coord y1, coord maxSnapDistance){
  coord lastSnapDistance = maxSnapDistance;
...
coord snap_y(coord sourceY, const objects_t& objects, const Grid& grid, coord x0, coord x1, coord maxSnapDistance){
  coord lastSnapDistance = maxSnapDistance;
