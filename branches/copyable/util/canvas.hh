...
class ToolInterface{
  ...
  // Canvas. The forwarding through ToolInterface allows the Canvas to
  // control additional actions, like refresh, which would not be
  // possible if the Tool-class was used directly.
public:
  virtual bool AcceptsPastedText() const = 0;

  virtual bool CopyText(utf8_string&) = 0;
  virtual bool CutText(utf8_string&) = 0;
...
public:
  ...
  virtual const Image& GetFrame(const index_t&) = 0;
  virtual const Image& GetFrameById(const FrameId&) = 0;
  virtual index_t GetFrameIndex(const Image&) const = 0;
  ...
  // command.
  virtual void SetMirage(const std::weak_ptr<Copyable<Bitmap>>&) = 0;
