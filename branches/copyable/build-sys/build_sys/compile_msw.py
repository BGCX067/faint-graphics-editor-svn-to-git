...
cl_common_switches = [
    ...
    # warnings in wxWidgets headers and Microsoft's last developer says its broken:
...
@print_timing
def compile(fileList,opts, out, err, debug):
...
@print_timing
def link(files, opts, debug):
    resFile = compile_resources(opts.project_root, opts.extra_resource_root)
    old = os.getcwd()
    ...
    if debug:
        flags += " /OUT:%s.exe /DEBUG /PDB:%s.pdb" % (opts.out_name,
                                                      opts.out_name)
    else:
        flags += " /OUT:%s.exe" % (opts.out_name)
    ...
    if opts.msw_subsystem == "windows":
        ...
        manifestCmd = 'mt.exe -manifest %s.exe.manifest -outputresource:%s.exe;1' % (opts.out_name, opts.out_name)
