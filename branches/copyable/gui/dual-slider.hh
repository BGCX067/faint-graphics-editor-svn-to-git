...
wxDECLARE_EVENT(EVT_FAINT_DUAL_SLIDER_CHANGE, DualSliderEvent);

// Creates a dual slider control
template<int MINB, int MAXB>
DualSlider* create_dual_slider(wxWindow* parent, const StaticBoundedInterval<MINB, MAXB>& values, const SliderBackground& bg){
  typedef StaticBoundedInterval<MINB, MAXB> RangeType;
  return DualSlider::Create(parent, make_closed_range(RangeType::min_allowed, RangeType::max_allowed), values.GetInterval(), bg);
}
