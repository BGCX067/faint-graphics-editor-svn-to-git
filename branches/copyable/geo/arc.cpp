...
AngleSpan::AngleSpan(const Angle& in_start, const Angle& in_stop)
  : start(in_start),
    stop(in_stop)
{}

int required_curve_count(const AngleSpan& angles){
  // Todo: Consider using Rotation instead of Angle
...
std::vector<Point> arc_as_path(const Tri& tri, const AngleSpan& angles){
  ...
  coord x = c.x + rxCosCurrentAngle * cosMainAngle - rySinCurrentAngle * sinMainAngle;
  coord y = c.y + rxCosCurrentAngle * sinMainAngle + rySinCurrentAngle * cosMainAngle;
  coord xDot = -rxSinCurrentAngle * cosMainAngle - ryCosCurrentAngle * sinMainAngle;
  coord yDot = -rxSinCurrentAngle * sinMainAngle + ryCosCurrentAngle * cosMainAngle;
