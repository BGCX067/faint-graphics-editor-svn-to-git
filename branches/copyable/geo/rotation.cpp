...
Rotation::Rotation(faint::radian radians)
  : m_radians(radians)
{}

Rotation Rotation::Rad(faint::radian a){
  return Rotation(a);
}

Rotation Rotation::FromAngle(const faint::Angle& a){
  return Rotation(a.Rad());
}

Rotation Rotation::Deg(faint::degree a){
...
faint::radian Rotation::Rad() const{
  return m_radians;
}

faint::degree Rotation::Deg() const{
  return m_radians * faint::degreePerRadian;
}
...
Rotation Rotation::operator/(faint::coord rhs) const{
  assert(rhs != 0);
...
Rotation abs(const faint::Rotation& a){
  return Rotation::Rad(std::fabs(a.Rad()));
}

faint::coord sin(const Rotation& a){
  return std::sin(a.Rad());
}

faint::coord cos(const Rotation& a){
  return std::cos(a.Rad());
}

faint::coord tan(const Rotation& a){
  return std::tan(a.Rad());
}

Rotation operator*(const Rotation& lhs, faint::coord rhs){
...
bool rather_zero( const faint::Rotation& rot ){
  return fabs(rot.Rad()) <= faint::coord_epsilon;
}
