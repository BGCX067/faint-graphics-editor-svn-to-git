...
PyFileFormat::PyFileFormat(const load_callback_t& loader, const save_callback_t& saver, const label_t& label, const FileExtension& ext)
  : Format(ext, label, can_save(saver.Get() != nullptr), can_load(loader.Get() != nullptr)),
    m_callLoad(nullptr),
    m_callSave(nullptr)
{
...
static utf8_string internal_load_error_prefix(const Format& format){
  return utf8_string("An Internal Error occured in the load function for ") + format.GetLabel() + utf8_string("\n\n");
}
