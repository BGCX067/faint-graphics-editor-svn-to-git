...
private:
  void UpdateStatus(StatusInterface& status){
    ...
    switch (m_state){
    case FIRST_POINT:
      status.SetMainText(space_sep(btn.This(true), "click to add curve,", btn.Other(true), "click to stop."));
      break;
    case CONTROL_1:
      status.SetMainText(space_sep(btn.This(true), "click to place first control point,", btn.Other(true), "click to stop."));
      break;
    case CONTROL_2:
      status.SetMainText(space_sep(btn.This(true), "click to place the second control point,", btn.Other(true), "click to stop."));
      break;
    }
  }
};
...
public:
  ...
  bool AllowsGlobalRedo() const override{
    return true; // Fixme, implement Path tool undo
  }
  ...
  ToolResult MouseUp(const PosInfo& info){
    return HandlePtResult(m_pointBuilder->MouseUp(info, m_path), info);
  }

  ToolResult MouseMove(const PosInfo& info){
  ...
  bool Set(const BoundSetting& s) override{
    ...
    if (changed && m_pointBuilder->Change(pointType, m_path)){
      m_pointBuilder.reset(CreatePointBuilder(m_pointBuilder->GetCurrentPos()));
    }
    return changed;
  }
...
private:
  ToolResult HandlePtResult(PtResult result, const PosInfo& info){
    ...
    case PtResult::POINT_DONE:
      m_pointBuilder.reset(CreatePointBuilder(info.pos));
      return TOOL_DRAW;
  ...
  ToolResult Commit(Layer layer){
    ...
    m_command.Set(add_or_draw(create_path_object(m_path, without(m_settings, ts_PointType, ts_ClosedPath)), layer));
    m_active = false;
    m_path.Clear();
    m_pointBuilder.reset(new MovePointBuilder());
    return TOOL_COMMIT;
  }

  PointBuilder* CreatePointBuilder(const Point& pos){
    PointType t = m_settings.Get(ts_PointType);
    if (t == PointType::LINE){
      return new LinePointBuilder(pos, m_path, m_mouseButton);
    }
    else{
      return new CurvePointBuilder(pos, m_path, m_mouseButton);
    }
  }
