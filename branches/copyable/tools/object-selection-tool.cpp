...
// Creates a single object update setting or a CommandBunch if
// multiple objects are affected
static Command* change_settings_objects(const objects_t& objects, const Settings& s){
  if (objects.empty()){
    return nullptr;
  }
  std::vector<Command*> commands;
  for (Object* obj : objects){
    commands.push_back(change_settings_command(obj, New(s), Old(obj->GetSettings())));
  }
  return perhaps_bunch(CommandType::OBJECT,
...
public:
  ObjectSelectionTool(const ActiveCanvas& activeCanvas)
    : MultiTool(ToolId::OBJECT_SELECTION,
      default_task(new SelectObjectIdle(m_settings, activeCanvas))),
  ...
  bool Set(const BoundSetting& s) override{
    ...
    s.Visit(
      ...
      [&](StrSetting s, StrSetting::ValueType v){
        cmd = change_setting_objects(objects, s, v);
        return true;
      },
