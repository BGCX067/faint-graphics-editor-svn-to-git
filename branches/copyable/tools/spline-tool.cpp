...
class SplineTool : public StandardTool{
public:
  ...
  void Draw(FaintDC& dc, Overlays&, const Point& currPos) override{
    if (m_active){
  ...
  IntRect GetRefreshRect(const IntRect&, const Point& currPos) const override{
    return floored(inflated(bounding_rect(m_p1, m_p2, currPos), GetSettings().Get(ts_LineWidth)));
  }
  ...
  ToolResult MouseDown(const PosInfo& info) override{
    if (m_active && m_otherButton == info.modifiers.MouseButton()){
      ...
      SetSwapColors(info.modifiers.RightMouse()); // Fixme: Pass info to swap instead.
...
private:
  ToolResult Commit(Layer layer){
    m_command.Set(add_or_draw(create_spline_object(m_points, GetSettings()), layer));
    m_states.Clear();
    m_points.Clear();
    m_active = false;
    return TOOL_COMMIT;
