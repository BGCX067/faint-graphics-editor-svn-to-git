...
static IntPoint get_x2y2(const CanvasResizeHandle& handle, const IntPoint& release){
...
static void set_rescale_status(StatusInterface& status, const CanvasResizeHandle& handle, const IntPoint& release, const ToolModifiers& modifiers){
  HandleDirection dir = handle.GetDirection();
...
static void set_resize_status(StatusInterface& status, const CanvasResizeHandle& handle, const IntPoint& pos, const ToolModifiers& modifiers){
  HandleDirection dir = handle.GetDirection();
...
public:
  ...
  ResizeCanvasTool(const CanvasResizeHandle& handle, Operation op, const Settings& allSettings)
    ...
    if (dir == HandleDirection::DIAGONAL){
      ...
      if (m_handle.GetPos().x > opposite.x){
        coord x = floated(m_release.x) + (m_release.x >= opposite.x ?
          1.0 : 0.0); // Offset to the right of the pixel if adjusting from the right
      ...
      if (m_handle.GetPos().y > opposite.y){
        coord y = floated(m_release.y) + (m_release.y >= opposite.y ?
          1.0 : 0.0); // Offset to the bottom of the pixel if adjusting from the bottom
  ...
  IntRect GetRefreshRect(const IntRect& r, const Point&) const override{
    HandleDirection dir = m_handle.GetDirection();
    IntPoint opposite(m_handle.Opposite().GetPos());
    if (dir == HandleDirection::DIAGONAL){
...
Tool* resize_canvas_tool(const CanvasResizeHandle& handle, const Settings& allSettings){
  return new ResizeCanvasTool(handle, ResizeCanvasTool::RESIZE, allSettings);
}

Tool* scale_canvas_tool(const CanvasResizeHandle& handle, const Settings& allSettings){
