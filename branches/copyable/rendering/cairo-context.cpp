...
cairo_pattern_t* faint_cairo_linear_gradient( const LinearGradient& g ){
  ...
  for ( int i = 0; i != g.GetNumStops().Get(); i++ ){
    faint::ColorStop stop(g.GetStop(index_t(i)));
...
faint::Bitmap cairo_skew( const faint::Bitmap& in_bmpSrc, faint::coord skew_angle, faint::coord skew_pixels ){
  ...
  faint::Bitmap bmpDst(IntSize(int(bmpSrc.m_w + std::fabs(skew_pixels)), int( bmpSrc.m_h)));
...
Bitmap rotate_bilinear( const Bitmap& in_bmpSrc, const faint::Angle& angle, const Paint& bg ){
  faint::Bitmap& bmpSrc(const_cast<faint::Bitmap&>(in_bmpSrc)); // Needs non-const bitmap for source surface. Will not modify.

  RotationAdjustment adj = get_rotation_adjustment(angle, in_bmpSrc.GetSize());
  faint::Bitmap bmpDst(adj.size, bg);
  cairo_surface_t* s_dst = get_cairo_surface( bmpDst );
  cairo_surface_t* s_src = get_cairo_surface( bmpSrc );

  cairo_t* cr = cairo_create( s_dst );
  cairo_translate(cr, bmpDst.m_w / 2.0, bmpDst.m_h / 2.0);
  cairo_rotate(cr, angle.Rad());
  cairo_set_source_surface( cr, s_src, -bmpSrc.m_w / 2.0, -bmpSrc.m_h / 2.0 );
  cairo_paint( cr );
  cairo_destroy( cr );
  cairo_surface_destroy( s_dst );
  cairo_surface_destroy( s_src );
...
Bitmap rotate_bilinear( const Bitmap& bmp, const faint::Angle& angle ){
  return rotate_bilinear( bmp, angle, faint::Paint(faint::color_transparent_white() ));
}

Bitmap rotate_scale_bilinear(const Bitmap& in_bmpSrc, const faint::Angle& angle, const Scale& scale, const faint::Paint& bg){
  faint::Bitmap& bmpSrc(const_cast<faint::Bitmap&>(in_bmpSrc)); // Needs non-const bitmap for source surface. Will not modify.

  RotationAdjustment adj = get_rotation_adjustment(angle, in_bmpSrc.GetSize());
  faint::Bitmap bmpDst(rounded(floated(adj.size) * scale), bg);
  cairo_surface_t* s_dst = get_cairo_surface( bmpDst );
  cairo_surface_t* s_src = get_cairo_surface( bmpSrc );
...
cairo_matrix_t cairo_linear_matrix_from_tri( const Tri& t, const faint::Angle& angle, bool objectAligned ){
  double sx = t.Width();
  double sy = t.Height();
  double x0 = t.P0().x;
  double y0 = t.P0().y;
...
cairo_matrix_t cairo_pattern_matrix_from_tri( const Tri& t, const Point& offset ){
  Point p0(t.P0());
  cairo_matrix_t m;
  cairo_matrix_init_identity(&m);
...
cairo_matrix_t translate_matrix( const Point& delta ){
  cairo_matrix_t m;
  cairo_matrix_init_identity(&m);
...
bool CairoContext::IsTargetting( const faint::Bitmap& bmp ) const{
...
void CairoContext::arc( const Point& c, faint::coord r, faint::coord angle1, faint::coord angle2 ){
  cairo_arc(m_impl->cr, c.x, c.y, r, angle1, angle2);
}

void CairoContext::arc_negative( const Point& c, faint::coord r, faint::coord angle1, faint::coord angle2 ){
  cairo_arc_negative(m_impl->cr, c.x, c.y, r, angle1, angle2);
}
...
void CairoContext::pango_text( const Tri& t, const utf8_string& text, const Settings& s){
  ...
  // but I've found no convenient way to turn that off when
...
std::vector<PathPt> CairoContext::get_text_path( const Tri& t, const utf8_string& text, const Settings& s){
  const bool bold = s.Has( ts_FontBold ) && s.Get( ts_FontBold );
  const bool italic = s.Has( ts_FontItalic ) && s.Get( ts_FontItalic );
  set_source( s.Get( ts_Fg ) ); // Fixme
  ...
  Point offset(0, ink.height / PANGO_SCALE);
  translate(offset);
  ...
  cairo_path_t* cairoPath = cairo_copy_path(m_impl->cr);
  ...
  for (int i = 0; i < cairoPath->num_data; i += cairoPath->data[i].header.length) {
    ...
    case CAIRO_PATH_MOVE_TO:
      path.push_back(PathPt::MoveTo(Point(data[1].point.x, data[1].point.y) + delta));
      break;
...
TextMeasures CairoContext::pango_text_extents( const utf8_string& text, const Settings& s ) const{
...
void CairoContext::rotate( const faint::Angle& angle ){
  cairo_rotate(m_impl->cr, angle.Rad());
}

void CairoContext::rotate( const faint::Rotation& rotation ){
  cairo_rotate(m_impl->cr, rotation.Rad());
}
...
void CairoContext::set_source( const faint::Paint& src ){
  dispatch(src,
...
void CairoContext::set_source_surface( const faint::Pattern& pattern ){
  if ( m_impl->srcSurface != nullptr ){
    cairo_surface_destroy(m_impl->srcSurface);
    cairo_pattern_destroy(m_impl->srcPattern);
  ...
  m_impl->srcBmp = new faint::Bitmap(copy(pattern.GetBitmap())); // Fixme: check format
  ...
  if ( m_impl->pattern->GetObjectAligned() ){
    cairo_matrix_t matrix = cairo_pattern_matrix_from_tri( m_impl->patternTri, floated(pattern.GetAnchor()));
...
void CairoContext::set_source_tri( const Tri& t ){
  ...
  if ( m_impl->linearGradient != 0 ){
    ...
    if ( objectAligned ){
      cairo_matrix_t matrix = cairo_pattern_matrix_from_tri(t, floated(m_impl->pattern->GetAnchor()));
      cairo_pattern_set_matrix(m_impl->srcPattern, &matrix);
    }
    else{
      cairo_matrix_t matrix = translate_matrix(floated(m_impl->pattern->GetAnchor()));
      cairo_pattern_set_matrix(m_impl->srcPattern, &matrix);
    }
    cairo_set_source( m_impl->cr, m_impl->srcPattern );
  }
