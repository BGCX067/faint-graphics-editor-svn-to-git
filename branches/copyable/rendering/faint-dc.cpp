...
static Paint get_bg(const Settings& s, const Point& origin, const IntPoint& clickPos){
  Paint src = get_bg(s);
...
static Paint get_fg(const Settings& s, const Point& origin, const IntPoint& clickPos){
  Paint src = get_fg(s);
  return offsat(src, -floored(origin), clickPos);
}

static BorderSettings border_settings(const Settings& s, const Paint& p){
  return {p, floored(s.Get(ts_LineWidth)), dashed(s)};
}
...
static BorderSettings border_settings(const Settings& s, const Point& origin, const IntPoint& clickPos){
  return border_settings(s, get_fg(s, origin, clickPos));
}

static LineSettings line_settings(const Settings& s, const Paint& p){
  return LineSettings(border_settings(s, p), get_line_cap(s));
}

static LineSettings line_settings(const Settings& s, const Point& origin){
  return LineSettings(border_settings(s, origin), get_line_cap(s));
}
...
static std::vector<Point> rectangle_as_clockwise_polygon(const Tri& tri){
  return {tri.P0(),
    tri.P1(),
    tri.P3(),
    tri.P2()};
...
void draw_arrow_head(CairoContext& cr, const Arrowhead& arrowHead, coord lineWidth, coord sc){
  Settings s;
...
static std::vector<Point> transform_points(const std::vector<Point>& points, coord scale, const Point& origin){
  std::vector<Point> v2;
...
void FaintDC::Blit(const Bitmap& bmp, const Point& topLeft, const Settings& settings){
...
void FaintDC::BitmapBlendAlpha(const Bitmap& drawnBitmap, const IntPoint& topLeft){
  if (m_sc < 1){
    const Bitmap scaled(scale_bilinear(drawnBitmap, Scale(m_sc)));
    blend(scaled, onto(m_bitmap), topLeft);
  }
  else if (m_sc > 1){
    ...
    blend(drawnBitmap, onto(m_bitmap), topLeft);
  }
}

void FaintDC::BitmapBlendAlphaMasked(const Bitmap& drawnBitmap, const Color& maskColor, const IntPoint& topLeft){
  if (m_sc < 1){
    const Bitmap scaled(scale_bilinear(drawnBitmap, Scale(m_sc)));
    blend_masked(scaled, onto(m_bitmap), maskColor, topLeft);
  }
  else if (m_sc > 1){
    const Bitmap scaled(scale_nearest(drawnBitmap, truncated(m_sc)));
    blend_masked(scaled, onto(m_bitmap), maskColor, topLeft);
    ...
    blend_masked(drawnBitmap, onto(m_bitmap), maskColor, topLeft);
  }
}

void FaintDC::BitmapSetAlpha(const Bitmap& drawnBitmap, const IntPoint& topLeft){
  if (m_sc < 1){
    const Bitmap scaled(scale_bilinear(drawnBitmap, Scale(m_sc)));
    blit(scaled, onto(m_bitmap), topLeft);
  }
  else if (m_sc > 1){
    const Bitmap scaled(scale_nearest(drawnBitmap, truncated(m_sc)));
    blit(scaled, onto(m_bitmap), topLeft);
    ...
    blit(drawnBitmap, onto(m_bitmap), topLeft);
  }
}

void FaintDC::BitmapSetAlphaMasked(const Bitmap& drawnBitmap, const Color& maskColor, const IntPoint& topLeft){
  if (m_sc < 1){
    Bitmap scaled(scale_bilinear(drawnBitmap, Scale(m_sc)));
    blit_masked(scaled, onto(m_bitmap), maskColor, topLeft);
  }
  else if (m_sc > 1){
    ...
    blit_masked(drawnBitmap, onto(m_bitmap), maskColor, topLeft);
  }
}

void FaintDC::Blend(const AlphaMap& alpha, const IntPoint& topLeft, const IntPoint& anchor, const Settings& s){
  ...
  if (f != nullptr){
    ...
    blend(bmp, offsat(alpha.FullReference(), imagePt), get_fg(s, m_origin, anchor));
    f->Apply(bmp);
    blend(bmp, onto(m_bitmap), imagePt - offset);
    delete f;
  }
  else{
    ...
    if (r.x + r.w > alphaSz.w || r.y + r.h >= alphaSz.h){
      blend(m_bitmap, offsat(alpha.FullReference(), imagePt), get_fg(s, m_origin, anchor));
    }
    else{
      blend(m_bitmap, offsat(alpha.SubReference(r), imagePt + r.TopLeft()),
...
void FaintDC::DrawRasterEllipse(const Tri& tri, const Filter& f, const Settings& s){
  IntRect r(floored(tri.P0() * m_sc + m_origin),
    floored(tri.P3() * m_sc + m_origin));
  ...
  if (filled(s)){
    fill_ellipse(bmp, r2,
      get_bg(s, m_origin + floated(offset - r.TopLeft()), rounded(center_point(tri))));
  }
  if (border(s)){
    draw_ellipse(bmp, r2,
  ...
  f.Apply(bmp);
  BitmapBlendAlpha(bmp, floored((tri.P0() - floated(offset)) * m_sc + m_origin));
}
...
void FaintDC::DrawRasterPolyLine(const std::vector<Point>& points, const Settings& s){
  ...
  if (s.GetDefault(ts_LineArrowhead, LineArrowhead::NONE) == LineArrowhead::FRONT){
    ...
    fill_triangle(m_bitmap, a.P0(), a.P1(), a.P2(), paint);
  }
...
void FaintDC::DrawRasterRect(const Tri& tri, const Filter& f, const Settings& s){
  IntRect r(floored(tri.P0() * m_sc + m_origin),
    floored(tri.P3() * m_sc + m_origin));
...
std::vector<PathPt> FaintDC::GetTextPath(const Tri& t, const utf8_string& text, const Settings& s){
...
void FaintDC::Line(const LineSegment& line, const Filter& f, const Settings& s){
  ...
  Bitmap bmp(truncated(Size(std::fabs(line.p1.x - line.p0.x), std::fabs(line.p1.y - line.p0.y))) + extraSize,
    color_transparent_white());
  IntPoint origin = floored(min_coords(line.p0, line.p1));
  IntPoint offset(10,10);
  draw_line(bmp, {floored(line.p0) - origin + offset, floored(line.p1) - origin + offset}, line_settings(s, m_origin));
  f.Apply(bmp);
...
// Based on http://commons.oreilly.com/wiki/index.php/SVG_Essentials/Paths#Elliptical_Arc
// and http://www.w3.org/TR/SVG/implnote.html#ArcImplementationNotes
CairoArc cairo_arc_from_svg_arc(const Point& p0, Radii r, coord xAxisRotation, int largeArcFlag, int sweepFlag, const Point& p){
...
void FaintDC::Path(const std::vector<PathPt>& points, const Settings& s){
  ...
  for (const PathPt& pt : points){
    if (pt.IsMove()){
      ...
      m_cr->save();
      coord angle = pt.axis_rotation / (180.0 / math::pi);
      CairoArc arc = cairo_arc_from_svg_arc(currPos, pt.r, angle, pt.large_arc_flag, pt.sweep_flag, pt.p);
      m_cr->translate(arc.center);
      m_cr->rotate(Rotation::Rad(angle));
      ...
      if (angle1 < angle2){
        m_cr->arc(Point(0,0), 1.0, angle1, angle2);
      }
      else{
        m_cr->arc_negative(Point(0,0), 1.0, angle1, angle2);
      }
      currPos = pt.p;
      m_cr->restore();
    }
...
void FaintDC::PenStroke(const std::vector<IntPoint>& points, const Settings& s){
  ...
  const Paint src = get_fg(s, m_origin);
  if (points.size() == 1){
...
void FaintDC::Text(const Tri& t, const utf8_string& text, const Settings& s, const Tri& clip){
  m_cr->save();
  m_cr->set_source_tri(t);
...
TextMeasures FaintDC::TextExtents(const utf8_string& text, const Settings& s) const{
  return m_cr->pango_text_extents(text, s);
}

std::vector<int> FaintDC::CumulativeTextWidth(const utf8_string& text, const Settings& s) const{
