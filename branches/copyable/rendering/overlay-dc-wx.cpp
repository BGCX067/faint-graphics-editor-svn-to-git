...
static void draw_corner( wxDC& dc, const faint::Angle& angle, const Point& pt, bool left, bool top ){
  faint::coord len(10.0);
...
static void draw_handle( wxDC& dc, const Point& p, faint::coord scale ){
  dc.DrawRectangle( truncated(p.x * scale - 8 / 2.0 + 0.5), truncated(p.y * scale - 8 / 2.0 + 0.5 ), 8, 8 );
}

static faint::Bitmap get_grid_bitmap( const Grid& grid, const IntPoint& imageRegionTopLeft, double scale, const IntSize& size ){
  faint::Bitmap bmp(size + IntSize(5,5), faint::Color(0,0,0,0));
  int spacing = rounded(grid.Spacing() * scale);
  const faint::Color color(grid.GetColor());
  ...
  const LineSettings s(color, width, dashed, LineCap::BUTT);
  for ( int i = 0; i != maxPos; i++ ){
    faint::coord x = i * spacing - xOff;
    draw_line(bmp, {floored(Point(x, 0)), floored(Point(x, bmp.m_h))}, s);

    faint::coord y = i * spacing - yOff;
...
void OverlayDC_WX::Corners( const Tri& tri ){
  ...
  brush.SetStyle( wxBRUSHSTYLE_TRANSPARENT );
  m_dc.SetBrush( brush );
  faint::Angle angle(tri.GetAngle());
