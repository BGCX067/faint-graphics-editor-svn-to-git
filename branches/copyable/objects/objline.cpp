...
public:
  ...
  faint::utf8_string StatusString() const override{
...
private:
  ...
  Rect GetLineRect() const {
    faint::coord lineWidth(std::max( m_settings.Get( ts_LineWidth ), 6.0 ));
    return inflated(bounding_rect(GetTri()), lineWidth / 2.0);
  }

  Rect GetArrowHeadRect() const {
    return get_arrowhead(LineSegment(m_tri.P0(), m_tri.P1()), m_settings.Get( ts_LineWidth ) ).BoundingBox();
  }
...
bool is_line(Object* obj){
  return dynamic_cast<ObjLine*>(obj) != nullptr;
}
