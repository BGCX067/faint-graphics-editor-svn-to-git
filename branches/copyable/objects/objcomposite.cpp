...
static void update_objects(const Tri& tri, const Tri& origTri, const objects_t& objects, const std::vector<Tri>& objTris){
  ...
  for (size_t objNum = 0; objNum != objects.size(); objNum++){
    ...
    temp = translated(Tri(p0, p1, p2), origTri.P0().x + a.tr_x, origTri.P0().y + a.tr_y);
...
public:
  ...
  std::vector<PathPt> GetPath() const override{
    ...
    for (Object* object : m_objects){
      std::vector<PathPt> subPath(object->GetPath());
