...
static void apply_transform(const Bitmap& src, const Tri& transform, Bitmap& dst){
  dst = src;
  Tri t2(transform);
  coord skew = t2.Skew();
  ...
  if (!rather_zero(skew)){
    ...
    Angle skew_angle = atan2(skew_pixels, (t2.P0().y - t2.P2().y) / (t2.Height() / src.m_h));
  ...
  if (!coord_eq(fabs(t2.Width()), src.m_w) || !coord_eq(fabs(t2.Height()), src.m_h)){
    ...
    dst = scale_bilinear(dst, Scale((t2.Width() + 1) / src.m_w, (t2.Height() + 1) / src.m_h));
  }
  else {
    if (t2.Width() < 0){
...
ObjRaster::ObjRaster(const ObjRaster& other)
  : Object(other.GetSettings()),
    m_bitmap(copy(other.m_bitmap)),
    m_scaled(copy(other.m_scaled)),
    m_tri(other.GetTri())
{}
...
std::vector<PathPt> ObjRaster::GetPath() const{
  return {
