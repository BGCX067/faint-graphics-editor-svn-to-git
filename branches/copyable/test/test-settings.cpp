...
void test_settings(){
  ...
  static_assert(std::is_move_constructible<Settings>::value,
    "Settings should be move constructible.");
  ...
  s.Set(str1, "First");
  s.Set(str2, "Second");
  VERIFY(s.Get(str1) == "First");
  VERIFY(s.Get(str2) == "Second");
  ...
  VERIFY(updated);
  VERIFY(s.Get(color1) == green);
  s.Set(color2, red);
  VERIFY(s.Get(color1) == green);
  VERIFY(s.Get(color2) == red);
  ...
  updated = s3.Update(s);
  VERIFY(updated);
  VERIFY(s3.Get(str1) == "First");
  VERIFY(!s3.Has(str2));
  s3 = s;
  VERIFY(s3.Get(str2) == "Second");
}
