...
def _stroke_and_or_fill( stream, object ):
    ...
    if fillStyle == 'border':
        stream.fgcol( *_to_pdf_color( object.get_fg() ) )
        stream.stroke()
    elif fillStyle == 'fill':
        stream.bgcol( *_to_pdf_color( object.get_fg() ) )
        stream.fill()
    elif fillStyle == 'border+fill':
        stream.fgcol( *_to_pdf_color( object.get_fg() ) )
        stream.bgcol( *_to_pdf_color( object.get_bg() ) )
...
def object_to_stream( s, object, doc_h, scale_x, scale_y ):
    ...
    if object.get_type() == 'Line':
        _set_dash( s, object )
        s.fgcol( *_to_pdf_color( object.get_fg() ) )
        ...
        if len( points ) == 4:
            ...
            if object.get_arrowhead() == 1:
                s.bgcol( *_to_pdf_color( object.get_fg() ) )
                arrowPos = _arrowhead( s, object.get_linewidth(), x0, y0, x1, y1 )
        ...
        _set_dash( s, object )
        s.linewidth( object.get_linewidth() )
        points = _transform_points( (p0[0], p0[1], p1[0], p1[1], p3[0], p3[1], p2[0], p2[1] ), doc_h, scale_x, scale_y )
        s.polygon( points )
        ...
        for num, item in enumerate(object.get_text_lines()):
            ...
            s.text(x, y - num * (textHeight + lineSpacing), object.get_fontsize(), textStr)
