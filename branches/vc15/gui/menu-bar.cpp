...
static void harmonize_zoom(const Canvas& source, AppContext& app){
  ...
  const index_t num(app.GetCanvasCount());
...
public:
  void Initialize(wxFrame& frame, AppContext& app, const ArtContainer& art){
    ...
    AddCheck(viewMenu,
      ...
      [&](wxCommandEvent& event){app.ShowToolPanel(event.IsChecked());});

    AddCheck(viewMenu,
      Label("&Status Bar", "Show or hide the status bar"), true,
      [&](wxCommandEvent& event){app.ShowStatusbar(event.IsChecked());});

    AddCheck(viewMenu,
      Label("&Color Panel", "Show or hide the color panel"), true,
      [&](wxCommandEvent& event){app.ShowColorPanel(event.IsChecked());});
    ...
    m_eventHandler.Bind(wxEVT_COMMAND_MENU_SELECTED,
      [&](wxCommandEvent&){app.GetActiveCanvas().ZoomDefault();}, FAINT_ZOOM_100);
  ...
  void UpdateZoom(const ZoomLevel& zoom){
    ...
    if (zoom.At100()){
      ...
      m_menuRef->m_menu->SetHelpString(FAINT_ZOOM_100_TOGGLE, "Fit image in view");
    }
    else {
      m_menuRef->m_menu->SetLabel(FAINT_ZOOM_100_TOGGLE, "Zoom 1:1\t*");
      m_menuRef->m_menu->SetHelpString(FAINT_ZOOM_100_TOGGLE, "Zoom to actual size");
    }
  }
