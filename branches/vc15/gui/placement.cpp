...
wxPoint to_the_left_middle_of(wxWindow* window, const wxSize& size){
  return wxPoint(window->GetPosition().x - size.GetWidth() - 5, window->GetPosition().y + window->GetSize().GetHeight() / 2 - size.GetHeight() / 2);
}

wxPoint to_the_right_middle_of(wxWindow* window, const wxSize& size){
  return wxPoint(window->GetPosition().x + window->GetSize().x + 5, window->GetPosition().y + window->GetSize().GetHeight() / 2 - size.GetHeight() / 2);
}
