  ...
  void UpdateZoom(const ZoomLevel& zoom){
    ...
    if (zoom.GetPercentage() == 100){
      m_btnZoomFitOr100->UpdateText("Fit",
        tooltip_t("Zoom to Fit"),
        description_t("Click to fit image in view, Ctrl=All images"));
      m_fit = true;
    }
    else {
      m_btnZoomFitOr100->UpdateText("1:1",
        tooltip_t("Use Actual Size"),
        description_t("Click to show image at 1:1, Ctrl=All images"));
      m_fit = false;
    }
