...
StatusButton::StatusButton(wxWindow* parent, const wxSize& size, StatusInterface& status, const utf8_string& label, const tooltip_t& tooltip, const description_t& description)
...
void StatusButton::UpdateText(const utf8_string& label, const tooltip_t& tooltip, const description_t& description){
  SetLabel(to_wx(label));
  m_description = description.Get();
  SetToolTip(to_wx(tooltip.Get()));
}

ToggleStatusButton::ToggleStatusButton(wxWindow* parent, int id,
  const wxSize& size,
