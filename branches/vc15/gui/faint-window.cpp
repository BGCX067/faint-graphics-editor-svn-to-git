...
// Contexts, included late to "inherit" the above includes
...
static index_t get_canvas_count(const FaintPanels& p){
  ...
  const PaintMap& palette)
{
  ...
  panels.tabControl = std::make_unique<TabCtrl>(&frame, app, app.GetStatusInfo());
  ...
  bool silentMode)
{
  ...
  frame->Bind(EVT_FAINT_ADD_TO_PALETTE,
    [&](const PaintEvent& event){
      AddToPalette(event.GetPaint());
  });

  frame->Bind(EVT_FAINT_OPEN_FILES,
    [&](const OpenFilesEvent& event){
      Open(event.GetFileNames());
    });

  frame->Bind(EVT_FAINT_TOOL_CHANGE,
  ...
  frame->Bind(EVT_FAINT_COPY_COLOR_HEX,
  ...
  frame->Bind(EVT_FAINT_COPY_COLOR_RGB,
    [this](const ColorEvent& event){
      Clipboard clipboard;
      if (!clipboard.Good()){
        show_copy_color_error(m_impl->frame.get());
  ...
  frame->Bind(EVT_FAINT_GRID_CHANGE,
  ...
  frame->Bind(EVT_SET_FOCUS_ENTRY_CONTROL,
    [this](const wxEvent&){
  ...
  frame->Bind(EVT_KILL_FOCUS_ENTRY_CONTROL,
    [this](const wxEvent&){
      EndTextEntry();
    });

  frame->Bind(EVT_FAINT_ACTIVE_CANVAS_CHANGE,
    [this](CanvasChangeEvent& e){
      auto& panels(*m_impl->panels);
  ...
  frame->Bind(EVT_FAINT_CANVAS_CHANGE,
      [this](CanvasChangeEvent& e){
        auto& panels(*m_impl->panels);
        auto& state(*m_impl->state);
        update_canvas_state(e.GetCanvasId(), panels, state);
  ...
  frame->Bind(EVT_FAINT_ZOOM_CHANGE,
    [this](CanvasChangeEvent& e){
      auto& panels(*m_impl->panels);
      if (e.GetCanvasId() == get_active_canvas(panels).GetId()){
        update_zoom(panels);
  ...
  frame->Bind(EVT_FAINT_LAYER_CHANGE,
  ...
  frame->Bind(wxEVT_CLOSE_WINDOW,
    [&](wxCloseEvent& event){
      ...
      panels.menubar->GetRecentFiles().Save();
      Clipboard::Flush();
  ...
  frame->Bind(EVT_TABLET,
    [=](const TabletEvent& event){
      appContext->TabletSetCursor(event.GetCursor());
    });

  m_impl->panels->tool->AsWindow()->Bind(EVT_FAINT_INT_SETTING_CHANGE,
    [this](const SettingEvent<IntSetting>& e){
      change_setting(*this, e.GetSetting(), e.GetValue(), from_control(true));
    });

  m_impl->panels->tool->AsWindow()->Bind(EVT_FAINT_FLOAT_SETTING_CHANGE,
    [this](const SettingEvent<FloatSetting>& e){
      change_setting(*this, e.GetSetting(), e.GetValue(), from_control(true));
    });

  m_impl->panels->tool->AsWindow()->Bind(EVT_FAINT_BOOL_SETTING_CHANGE,
    [this](const SettingEvent<BoolSetting>& e){
      change_setting(*this, e.GetSetting(), e.GetValue(), from_control(true));
    });

  m_impl->panels->tool->AsWindow()->Bind(EVT_FAINT_SETTINGS_CHANGE,
  ...
  m_impl->panels->color->AsWindow()->Bind(EVT_FAINT_COLOR_SETTING_CHANGE,
...
void FaintWindow::CloseDocument(Canvas& canvas){
  ...
  for (index_t i(0); i != tabControl.GetCanvasCount(); ++i){
...
Canvas& FaintWindow::GetCanvas(const index_t& i){
  return m_impl->panels->tabControl->GetCanvas(i)->GetInterface();
}

index_t FaintWindow::GetCanvasCount() const{
  ...
  const change_tab& changeTab)
{
  ...
  for (Format* format : loading_file_formats(m_impl->state->formats)){
    if (format->Match(extension)){
      ...
      panels.menubar->GetRecentFiles().Add(filePath);
  ...
  return maybeFilePath.Visit(
    [&](const FilePath& p){
      ...
      if (format->CanLoad() || get_load_format(state.formats,
          p.Extension()) != nullptr){
        m_impl->panels->menubar->GetRecentFiles().Add(p);
      }
      return true;
    },
    [&](){
