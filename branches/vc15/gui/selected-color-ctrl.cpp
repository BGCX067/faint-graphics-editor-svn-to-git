...
static Color border_color(const FlaggedPaint& p){
  // - With solid border, black works fine regardless of the surrounded
  // color, as the border is just a decoration.
  //
  // - With a dashed border, use a brighter border around darker colors to
  // ensure its easily visible, as the dashes indicate that the color
  // targets a selection.
      ...
      m_statusInfo(statusInfo)
  {
    ...
    Bind(wxEVT_COMMAND_MENU_SELECTED,
      [this](wxCommandEvent& event){
        HandleMenuChoice(event.GetId());
      });
  }

  void UpdateColors(const ColorChoice& colors){
    m_fgBmp = selected_color_bitmap(colors.fg, to_faint(m_fgRect.GetSize()));
    m_bgBmp = selected_color_bitmap(colors.bg, to_faint(m_bgRect.GetSize()));
