...
public:
      ...
      m_statusInterface(status)
  {
    ...
    Bind(wxEVT_LEFT_DOWN,
      [&](wxMouseEvent& event){
        m_dragStart = to_faint(event.GetPosition());
        CellPos pos = MousePosToPalettePos(m_dragStart);
        if (!m_paintMap.Has(pos)){
          return;
        }

        Paint fg(m_paintMap.Get(pos));
        SetFg(fg);
    ...
    auto rightClickLambda([&](wxMouseEvent& event){
    ...
    Bind(wxEVT_RIGHT_DOWN, rightClickLambda);
    Bind(wxEVT_RIGHT_DCLICK, rightClickLambda);
  }
