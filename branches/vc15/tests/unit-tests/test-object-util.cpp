...
void test_object_util(){
  ...
  {
    // Test "area"
    VERIFY(rounded(object_area(rect.get())) == 50);
...

  {
    ...
    VERIFY(rounded(r.TopLeft()) == IntPoint(0,0));
    VERIFY(rounded(r.BottomRight()) == IntPoint(20,10));
    // Test "get_collective_name"
    VERIFY(get_collective_type(LIST(rect.get(), polygon.get())) == "Objects");

    VERIFY(get_collective_type(LIST(rect.get())) == "Rectangle");

    VERIFY(get_collective_type(LIST(polygon.get())) == "Polygon");

    std::unique_ptr<Object> r2(create_rectangle_object(Tri(Point(0,0),
      Point(10,0), Point(0,10)),
      default_rectangle_settings()));
    VERIFY(get_collective_type(LIST(rect.get(), r2.get())) == "Rectangles");
  }

  {
    ...
    NOT(rect->GetSettings().Has(ts_EditPoints));
    NOT(toggle_edit_points(rect.get()));
    NOT(rect->GetSettings().Has(ts_EditPoints));
    NOT(supports_point_editing(rect.get()));
...

  {
    ...
    VERIFY(rounded(prev_point(polygon.get(), 0)) == IntPoint(20,10));
    VERIFY(rounded(prev_point(polygon.get(), 1)) == IntPoint(0,10));
    VERIFY(rounded(prev_point(polygon.get(), 2)) == IntPoint(10,0));

    VERIFY(rounded(next_point(polygon.get(), 0)) == IntPoint(10,0));
    VERIFY(rounded(next_point(polygon.get(), 1)) == IntPoint(20,10));
    VERIFY(rounded(next_point(polygon.get(), 2)) == IntPoint(0,10));
  }

  {
    ...
    VERIFY(rounded(prev_point(line.get(), 0)) == IntPoint(10,0));
    VERIFY(rounded(prev_point(line.get(), 1)) == IntPoint(0,10));
    VERIFY(rounded(prev_point(line.get(), 2)) == IntPoint(10,0));

    VERIFY(rounded(next_point(line.get(), 0)) == IntPoint(10,0));
    VERIFY(rounded(next_point(line.get(), 1)) == IntPoint(20,10));

    // Weird: Next at end becomes prev for lines (non-cyclic).
    VERIFY(rounded(next_point(line.get(), 2)) == IntPoint(10,0));
  }
