...
void test_optional(){
  ...
  VERIFY(optional.Or(alt).GetSize() == altSize);
  ...
  VERIFY(optional.IsSet());
  VERIFY(optional);
  VERIFY(optional.Or(alt).GetSize() == bmpSize);
  optional.Visit(FAIL_UNLESS_CALLED());
  optional.Visit(FAIL_UNLESS_CALLED(), FAIL_IF_CALLED());
  IntSize sz = optional.VisitSimple([](const Bitmap& bmp){return bmp.GetSize();}, alt.GetSize());
  VERIFY(sz == bmpSize);
  VERIFY(optional.Get().GetSize() == bmpSize);
  ...
  VERIFY(optional2.Get().GetSize() == bmp.GetSize());
  optional2.Clear();
  ...
  o.Get().value++;
  VERIFY(o.Get().value == 11);

  Optional<FailIfCopied&> o2(o);
  VERIFY(o2.IsSet());
  o2.Get().value++;
  VERIFY(o.Get().value == 12);

  IntHolder h(12);
  VERIFY(h.value == 12);
  h.Get(true).Get()++;
  VERIFY(h.value == 13);
  VERIFY(h.Get(true).Get() == 13);
}
