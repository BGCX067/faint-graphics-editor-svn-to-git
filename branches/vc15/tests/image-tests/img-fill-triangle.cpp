...
void img_fill_triangle(){
  ...
  const degree step(360.0 / NUM_IMAGES);

  for (int i = 0; i != NUM_IMAGES; i++){
    Point p0r = rotate_point(p0, Angle::Deg(step * i), c);
    Point p1r = rotate_point(p1, Angle::Deg(step * i), c);
    Point p2r = rotate_point(p2, Angle::Deg(step * i), c);
