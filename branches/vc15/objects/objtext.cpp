...
public:
  ...
  coord ComputeRowHeight() const override{
    if (m_rowHeight.NotSet()){
      m_rowHeight.Set(m_dc.TextSize(utf8_string("M"), m_settings).h);
    }
...
coord ObjText::BaselineOffset() const{
  ...
  TextMeasures rects(info.TextExtents(utf8_string(1, utf8_char("M"))));
  return rects.ink.h;
}
...
IntRect ObjText::GetRefreshRect() const{
  if (m_settings.Get(ts_BoundedText)){
    ...
    coord w = 0;
    coord h = info.ComputeRowHeight() * lines.size();
    for (auto l : lines){
