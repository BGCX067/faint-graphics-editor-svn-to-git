...
struct sizeof_rest{
  static const size_t value = sizeof(typename std::tuple_element<N, std::tuple<Head, Tail...> >::type) + sizeof_rest<N - 1, Head, Tail...>::value;
};
...
struct sizeof_entries{
  static const size_t value = sizeof_rest<std::tuple_size< std::tuple<Head, Tail...> >::value - 1, Head, Tail...>::value;
};
...
auto serialize_tuple(const std::tuple<Head, Tail...>& t){
  std::array<unsigned char, sizeof_entries<Head, Tail...>::value> a = {0};
