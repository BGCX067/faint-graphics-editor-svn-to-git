...
FrameInfo::FrameInfo(const delay_t& in_delay, const HotSpot& hotSpot)
  : delay(in_delay.Get()),
    hotSpot(hotSpot)
{}
...
index_t FrameProps::AddObject(Object* obj){
...
Object* FrameProps::GetObject(const index_t& index){
  return m_allObjects.at(to_size_t(index));
}

bool FrameProps::HasObject(const index_t& index) const{
  return index < to_index(m_allObjects.size());
}

bool FrameProps::IsTopLevel(const index_t& index) const{
