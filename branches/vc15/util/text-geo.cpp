  ...
  const Align& align)
{
  ...
  for (size_t row = s0.row; row <= s1.row; row++){
    coord dx = row * rowHeight * sin(-tri.GetAngle());
    coord dy = row * rowHeight * cos(tri.GetAngle());
  ...
  const Align& align)
{
  ...
  for (size_t row = 0; row != lines.size(); row++){
    ...
    coord dx = (row * rowHeight + yOffset) * sinAngle;
    coord dy = (row * rowHeight + yOffset) * cosAngle;
...
Size text_extents(const TextInfo& info, const text_lines_t& lines){
  ...
  return Size(width, info.ComputeRowHeight() * lines.size());
}
