...
Point adjust_to(const Point& o, const Point& p, const Rotation& step, const Rotation& offset){
  const int n_angles = 360 / truncated(step.Deg()) + 1;
  const coord length = distance(o, p);
  const Rotation angle = Rotation::FromAngle(angle360({o, p}));

  for (int i = 0; i!= n_angles; i++){
    Rotation i_angle = i * step + offset;
    if (abs(angle - i_angle) <= step / 2.0){
      Rotation newAngle = 2 * Rotation::FromAngle(pi) - i_angle;
...
Point adjust_to_default(const Point& origin, const Point& p, const Angle& angle, const Angle& altAngle){
  radian twoPi = 2 * pi.Rad();
  int n_angles = static_cast<int>(twoPi / angle.Rad()) + 1;
  coord length = distance(origin, p);
  radian initialAngle = -line_angle({origin, p}).Rad();
  if (initialAngle < 0){
    initialAngle = twoPi + initialAngle;
  }

  radian foundAngle = initialAngle;
  for (int i = 0; i!= n_angles; i++){
    radian thisAngle = i * angle.Rad();
    if (std::fabs(radian(initialAngle - thisAngle)) <= angle.Rad() / 2){
      foundAngle = thisAngle;
      break;
    }
  }

  if (std::fabs(initialAngle - altAngle.Rad()) < std::fabs(initialAngle - foundAngle)){
    // Use the altAngle instead
    foundAngle = altAngle.Rad();
  }

  radian newAngle = -foundAngle;
...
Point constrain_proportional(const Point& moved, const Point& opposite, const Point& origin){
  ...
  return fabs(x2) > fabs(moved.x) ?
    Point(x2, moved.y) : Point(moved.x, y2);
}
