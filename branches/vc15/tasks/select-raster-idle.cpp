...
TaskResult move_floating_content(const PosInfo& info, PendingCommand& command, PendingTask& newTask, Settings& settings, const SELECTION_TYPE& s, Canvas& canvas){
...
static Color selection_or_image_color(const RasterSelection& selection, const Image& image, IntPoint pos){
  return sel::visit(selection,
