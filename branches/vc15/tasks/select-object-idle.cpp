      ...
      [&](const Handle& handle) -> utf8_string{
        if (corner_handle(handle)){
          return info.modifiers.Primary() ?
            ...
            Sentence("Click to resize the", object_type(info)) + "Ctrl=Rotate";
        }
        else{
...
public:
  IdleSelectionState(Settings& in_settings, const ActiveCanvas& in_canvas)
    : canvas(in_canvas),
...
static TaskResult clicked_unselected(IdleSelectionState& impl, const PosInfo& info){
...
static TaskResult clone_selected_objects(IdleSelectionState& impl, Canvas& canvas, const Point& mousePos){
...
static TaskResult move_selected_objects(IdleSelectionState& impl, const objects_t& objects, const Point& mousePos){
...
static TaskResult select_object_rectangle(IdleSelectionState& impl, const Point& mousePos){
