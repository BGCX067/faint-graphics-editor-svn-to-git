...
#include "bitmap/quantize.hh" // Fixme: Shouldn't quantize here.
...
int and_map_stride(int w){
  return ((w % 32) == 0) ? w / 8 :
    4 * (w / 32 + 1);
}

int and_map_len(const IntSize& bmpSize){
...
void write_8bpp_BI_RGB(BinaryWriter& out, const Bitmap& bmp){
  auto indexed = quantized(bmp, Dithering::ON);
  const ColorList& palette = indexed.second;
  ...
  for (int y = 0; y != sz.h; y++){
    for (int x = 0; x != sz.w; x++){
      out.put(indexMap.Get(x, sz.h - y - 1));
    }
    for (int i = 0; i != padBytes; i++){
      out.put(0);
    }
...
void write_8bpp_BI_RGB_grayscale(BinaryWriter& out, const Bitmap& bmp){
  int padBytes = bmp_row_padding<8>(bmp.m_w);
  for (int y = 0; y != bmp.m_h; y++){
    for (int x = 0; x != bmp.m_w; x++){
      Color c(get_color_raw(bmp, x, bmp.m_h - y - 1));
      out.put(static_cast<char>(sum_rgb(c) / 3));
    }
    for (int i = 0; i != padBytes; i++){
      out.put(0);
    }
...
Optional<Bitmap> read_24bpp_BI_RGB(BinaryReader& in, const IntSize& size){
  Bitmap bmp(size, color_blue()); // Fixme
