...
static LogicalScreenDescriptor read_logical_screen_descriptor(BinaryReader& f){
...
static void read_color_table(BinaryReader& f, ColorTable& table){
...
static Bitmap get_bg(ImageProps& props, const LogicalScreenDescriptor& lsd, const ColorTable& /*colorTable*/, const DisposeInfo& disposeInfo){
  ...
  default:
    return props.GetFrame(props.GetNumFrames() - 1).GetBackground().Expect<Bitmap>();
  }
}
...
static bool read_block(BinaryReader& f, ImageProps& props, LogicalScreenDescriptor& lsd, ColorTable& globalColorTable, GraphicControlExtension& gce, DisposeInfo& disposeInfo){
  ...
  if (identifier == 0x21){
    ...
    Bitmap bg = get_bg(props, lsd, colorTable, disposeInfo);
...
void read_gif(const FilePath& filePath, ImageProps& props){
  try{
    ...
    if (props.GetNumFrames() > 1 && gifVersion == GifVer::GIF87a){
      props.AddWarning(utf8_string("GIF specified as GIF87a, but contains multiple frames."));
    }
  }
