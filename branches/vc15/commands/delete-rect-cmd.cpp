...
public:
  DeleteRectCommand(const IntRect& rect, const Paint& bg, const Optional<Paint>& altBg, bool dwim)
...
Command* get_delete_rect_command(const IntRect& rect, const Paint& bg){
  return new DeleteRectCommand(rect, bg, no_option(), false);
}

Command* get_delete_rect_command(const IntRect& rect, const Paint& bg, const Alternative<Paint>& altBg){
