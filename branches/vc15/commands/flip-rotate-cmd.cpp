...
static Point get_object_offset(const Rect& imageRect, const Angle& angle, const Scale& scale){
...
public:
  ...
  void Do(CommandContext& ctx) override{
    ...
    for (Object* obj : ctx.GetObjects()){
      obj->SetTri(translated(rotated(scaled(obj->GetTri(), m_scale, rect.Center()), m_angle, rect.Center()),
          -m_offset.x, -m_offset.y));
    }
    DoRaster(ctx);
  }
...
Command* rotate_scale_image_bilinear_command(const Angle& angle, const Scale& scale, const Paint& bg){
