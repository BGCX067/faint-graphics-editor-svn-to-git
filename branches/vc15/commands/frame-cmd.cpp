...
public:
  ...
  AddFrameCommand(const Image& frame, const index_t& index)
    : Command(CommandType::FRAME),
...
Command* add_frame_command(const Image& image, const index_t& index){
  return new AddFrameCommand(image, index);
}

class RemoveFrameCommand : public Command {
public:
  RemoveFrameCommand(const index_t& index)
    : Command(CommandType::FRAME),
      m_image(nullptr),
      m_index(index)
  {}
...
static void swap_frames(CommandContext& ctx, const index_t& f1, const index_t& f2){
  Image* img1 = &ctx.GetFrame(f1);
  Image* img2 = &ctx.GetFrame(f2);
  ctx.RemoveFrame(f2);
  ctx.AddFrame(img2, index_t(f1.Get() + 1));
  ctx.RemoveFrame(f1);
...
public:
  SwapFramesCommand(const index_t& f1, const index_t& f2)
...
Command* swap_frames_command(const index_t& f1, const index_t& f2){
  return new SwapFramesCommand(std::min(f1, f2), std::max(f1, f2));
}

class ReorderFrameCommand : public Command {
public:
  ReorderFrameCommand(const new_index_t& newIndex, const old_index_t& oldIndex)
    : Command(CommandType::FRAME),
      m_newIndex(newIndex),
      m_oldIndex(oldIndex)
  {}
...
public:
  SetFrameDelayCommand(const index_t& frameIndex, const NewDelay& newDelay, const OldDelay& oldDelay)
    : Command(CommandType::FRAME),
...
public:
  SetFrameHotSpotCommand(const index_t& frameIndex, const NewHotSpot& newHotSpot, const OldHotSpot& oldHotSpot)
    : Command(CommandType::FRAME),
      m_frameIndex(frameIndex),
