...
public:
  CommandBunch(CommandType type, const commands_t& commands, const bunch_name& name, MergeCondition* mergeCondition=nullptr)
    : Command(type),
      m_commands(commands),
      m_name(name.Get()),
...
Command* perhaps_bunch(CommandType type, const bunch_name& name, const std::deque<Command*>& commands){
  assert(!commands.empty());
  return (commands.size() == 1) ? commands.front() :
    new CommandBunch(type, std::vector<Command*>(begin(commands), end(commands)), name);
}

Command* command_bunch(CommandType type, const bunch_name& name, const commands_t& commands, MergeCondition* mergeCondition){
...
Command* command_bunch(CommandType type, const bunch_name& name, Command* cmd, MergeCondition* mergeCondition){
  return new CommandBunch(type, {cmd}, name, mergeCondition);
}

Command* command_bunch(CommandType type, const bunch_name& name, Command* cmd1, Command* cmd2, MergeCondition* mergeCondition){
