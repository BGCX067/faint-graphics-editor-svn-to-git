...
class PaintImpl{
public:
  PaintImpl(const Color& in_color)
    : color(in_color)
  {}
  PaintImpl(const Pattern& in_pattern)
    : pattern(in_pattern)
  {}
  PaintImpl(const Gradient& in_gradient)
...
Paint::Paint(){
  m_impl = new PaintImpl(Color(0,0,0));
}
...
Color get_color_default(const Paint& paint, const Color& defaultColor){
  return paint.IsColor() ? paint.GetColor() :
    defaultColor;
}
