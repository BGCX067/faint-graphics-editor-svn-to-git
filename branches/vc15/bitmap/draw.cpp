...
static void blend_gradient(const Offsat<AlphaMapRef>& in_alphaMap, DstBmp dst,
  const Gradient& g)
{
  ...
  alphaMap.BoundingRect().Visit(
    [&](const IntRect& r){
      ...
      blend(offsat(strokeMap, in_alphaMap.Offset()), dst);
    });
}

static void blend_color(const Offsat<AlphaMapRef>& in_alphaMap, DstBmp dst,
  const Color& c)
{
  const AlphaMapRef& alphaMap(in_alphaMap.Get());
  ...
  const Gradient& gradient, const Color& boundaryColor)
{
  ...
  Color fillColor = boundaryColor == color_black() ? color_white() :
    color_black();
  ...
  const Gradient& gradient)
{
  std::vector<IntPoint> v;
  const Color targetColor = get_color(bmp, pos);
  Color fillColor = targetColor == color_black() ? color_white() :
    color_black();
  ...
  const Optional<BorderSettings>& fg, const Optional<Paint>& bg)
{
  IntRect r(fg.VisitSimple(
    [&](const BorderSettings& s){
      return IntRect(s.Dashed() ? deflated(inRect, s.lineWidth/2) : inRect);
    },
    inRect));
...
void vertical_scanline(Bitmap& bmp, int x, const Color& c){
  draw_line(bmp, {{x, 0}, {x, bmp.m_h}}, {c, 1, LineStyle::SOLID, LineCap::BUTT});
}

void horizontal_scanline(Bitmap& bmp, int y, const Color& c){
  draw_line(bmp, {{0, y}, {bmp.m_w, y}}, {c, 1, LineStyle::SOLID, LineCap::BUTT});
}
