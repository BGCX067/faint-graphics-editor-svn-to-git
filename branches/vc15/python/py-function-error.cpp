...
void throw_if_outside(const index_t& index, const index_t& bound){
  if (bound <= index){
...
TypeError::TypeError(const TypeName& expectedType, const utf8_string& extraInfo, int n)
  : PythonError(PyExc_TypeError,
    space_sep("Argument", str_int(n + 1),
      "must be a",
      expectedType.Get(),
      no_sep(bracketed(extraInfo), ".")))
{}
