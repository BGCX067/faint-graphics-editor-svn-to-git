...
// settings.
static Settings specific_or_app(const Canvas& c,const Settings& base, const Optional<Settings>& maybe){
  return maybe.Visit(
     [&](const Settings& s){
...
static void canvas_ellipse(Canvas& canvas, const Rect& r, const Optional<Settings>& maybe){
...
static Bitmap canvas_get_stamped_bitmap(Canvas& canvas){
  return stamp_selection(canvas.GetImage());
}

/* method: "get_frame([index])\n
Returns the frame with the specified index, or the selected frame if
no index specified." */
static Frame canvas_get_frame(Canvas& canvas, const Optional<index_t>& num){
  return num.Visit([&canvas](const index_t& index){
...
static std::vector<Frame> canvas_get_frames(Canvas& canvas){
  ...
  frames.reserve(to_size_t(numFrames));
  for (int i = 0; i != numFrames; i++){
    frames.push_back(Frame(&canvas, canvas.GetFrame(index_t(i))));
  }
  return frames;
}
...
static BoundObjects canvas_get_objects(Canvas& canvas){
  ...
  auto frameId = image.GetId();

  BoundObjects boundObjects;
  for (Object* obj : objects){
    boundObjects.emplace_back(&canvas, obj, frameId);
  }
  return boundObjects;
}
...
static std::vector<BoundObject<Object>> canvas_get_selected(Canvas& canvas){
  ...
  std::vector<BoundObject<Object>> boundObjects;
  for (Object* obj : objects){
    boundObjects.emplace_back(&canvas, obj, frameId);
  }
  return boundObjects;
...
static BoundObject<Object> canvas_Ellipse(Canvas& canvas, const Rect& bounds, const Optional<Settings>& s){
...
static BoundObject<Object> canvas_Rect(Canvas& canvas, const Rect& r, const Optional<Settings>& s){
  return canvas_add_object(canvas,
...
static void canvas_rect(Canvas& canvas, const Rect& r, const Optional<Settings>& maybe){
  Settings s(with(specific_or_app(canvas, default_rectangle_settings(), maybe),
    ts_AntiAlias, false));
  Command* cmd = draw_object_command(
    its_yours(create_rectangle_object(tri_from_rect(r), s)));
  python_run_command(canvas, cmd);
}
...
static void canvas_select_frame(Canvas& self, const index_t& index){
...
static void canvas_set_size(Canvas& canvas, const IntSize& sz, const Optional<Paint>& maybePaint){
...
static void canvas_swap_frames(Canvas& canvas, const index_t& f0, const index_t& f1){
  const index_t numFrames(canvas.GetNumFrames());
...
static void canvas_brightness_contrast(Canvas& canvas, double brightness, double contrast){
