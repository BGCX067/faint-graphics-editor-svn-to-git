...
void throw_if_missing(const BoundFrameProps& self, const index_t& objectId){
...
static index_t frameprops_Group(const BoundFrameProps& self,
  const std::vector<index_t>& ids)
{
  ...
  objects_t objects;
  for (const index_t& objectId : ids){
  ...
  index_t groupId = self.frame.AddObject(composite);
  return groupId;
}

/* method: "Ellipse(x,y,w,h, settings) -> id\n
Creates an ellipse object and returns its id." */
static index_t frameprops_Ellipse(const BoundFrameProps& self, const Rect& bounds, const Settings& s){
...
static index_t frameprops_Line(const BoundFrameProps& self,
...
id of the path." */
static index_t frameprops_Path(const BoundFrameProps& self,
...
polygon id." */
static index_t frameprops_Polygon(const BoundFrameProps& self, const std::vector<coord>& coords, const Settings& s){
...
its id.\n" */
static index_t frameprops_Raster(const BoundFrameProps& self,
  const Rect& r, const Bitmap& bmp, const Settings& s)
{
...
static index_t frameprops_Rect(const BoundFrameProps& self, const Rect& r, const Settings& s, const Optional<utf8_string>& name){
  index_t index = self.frame.AddObject(create_rectangle_object(tri_from_rect(r),
...
static index_t frameprops_Spline(const BoundFrameProps& self,
  const std::vector<coord>& coords,
  const Settings& s)
{
  if (coords.empty()){
...
static index_t frameprops_Text(const BoundFrameProps& self, const Rect& r, const utf8_string& text, const Settings& s){
...
static Tri frameprops_get_obj_tri(const BoundFrameProps& self, const index_t& objectId){
  throw_if_missing(self, objectId);
...
static coord frameprops_get_obj_text_height(const BoundFrameProps& self, const index_t& objectId){
  throw_if_missing(self, objectId);
...
color." */
static void frameprops_set_background(const BoundFrameProps& self, const Either<Bitmap, ColorSpan>& bg){
