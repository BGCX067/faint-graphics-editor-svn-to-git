...
const TypeName arg_traits<index_t>::name("Index");
...
static PyObject* build_paint(const Paint& paint){
  return dispatch(paint, build_color_tuple, build_pattern, build_gradient);
}
...
bool parse_index_t(PyObject* args, int n, index_t* value){
  ...
  *value = index_t(static_cast<int>(temp)); // Fixme: Use conversion function
  return true;
}
...
bool parse_coord(PyObject* args, int n, coord* value){
  if (PySequence_Check(args)){
    ...
    *value = PyFloat_AsDouble(args);
    ...
    throw TypeError(arg_traits<coord>::name, n); // TODO: Use traits here too
  }
}
...
bool parse_flat(Calibration& calibration, PyObject* args, int& n, int len){
  ...
  PyObject* unitStr;
  if (!PyArg_ParseTuple(args, "(dddd)dO", &x0, &y0, &x1, &y1, &lineLength, &unitStr)){
...
bool parse_flat(ColorSpan& colorSpan, PyObject* args, int& n, int len){
  ...
  if (!parse_flat(size, maybeSize.get(), n3,
      PySequence_Length(maybeSize.get())))
  {
