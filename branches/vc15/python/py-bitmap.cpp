...
static void Bitmap_line(Bitmap& self, const IntLineSegment& line, const Color& c){
  draw_line(self, line, LineSettings(c, 1, LineStyle::SOLID, LineCap::BUTT));
}
...
template<>
void Common_aa_line<Bitmap&>(Bitmap& bmp, const IntLineSegment& line, const ColRGB& color){
...
template<>
void Common_boundary_fill(Bitmap& bmp, const IntPoint& pos, const Paint& fill, const Color& boundary){
...
template<>
void Common_color_balance(Bitmap& bmp, const color_range_t& r, const color_range_t& g, const color_range_t& b){
...
template<>
void Common_erase_but_color(Bitmap& bmp, const Color& keep, const Optional<Paint>& eraser){
  if (eraser.NotSet()){
...
template<>
void Common_replace_color(Bitmap& bmp, const Color& old, const Paint& replacement){
  replace_color(bmp, Old(old), replacement);
}

template<>
void Common_rotate(Bitmap& bmp, const radian& radians, const Optional<Paint>& bg){
  if (bg.NotSet()){
    throw ValueError("No background specified!");
  }
  bmp = rotate_bilinear(bmp, Angle::Rad(radians), bg.Get());
}
...
template<>
void Common_set_threshold(Bitmap& bmp, const threshold_range_t& range, const Optional<Paint>& in, const Optional<Paint>& out){
  if (in.NotSet()){
