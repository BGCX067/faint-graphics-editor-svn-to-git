...
static std::vector<Canvas*> list_images(){
  AppContext& app = get_app_context();
  index_t numCanvas = app.GetCanvasCount();
  std::vector<Canvas*> v;
  v.reserve(to_size_t(numCanvas));
  for (index_t i(0); i != numCanvas; ++i){
    v.push_back(&app.GetCanvas(i));
  }
  return v;
}
...
Returns a list of the available file formats." */
static std::vector<std::pair<utf8_string,utf8_string>> list_formats(){
...
name: "blit" */
static void py_blit_bitmap(const Bitmap& src, const IntPoint& topLeft, bitmapObject*& dst){
  blit(offsat(src, topLeft), onto(*(dst->bmp)));
}

/* function: "tool([i])\nSelects the tool with the specified id, or returns the current id if none specified." */
...
static bool parse_jpg_bitmap(PyObject* args, Bitmap& out){
  ...
  Py_ssize_t len = PyBytes_Size(pngStrPy);
  Bitmap bmp(from_jpg(PyBytes_AsString(pngStrPy), to_size_t(len)));
  py_xdecref(pngStrPy);
  out = bmp;
  return true;
}
