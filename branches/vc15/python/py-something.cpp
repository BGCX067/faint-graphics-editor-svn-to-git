...
supported by groups." */
static BoundObject<Object> Smth_get_obj(const BoundObject<Object>& self, const int& index){
  ...
  return BoundObject<Object>(self.canvas, self.obj->GetObject(index), self.frameId);
}
...
/* method: "num_objs()->i\n Returns the number of sub-objects. Only
useful for Group objects." */
...
/* method: "rotate(a[,(x0,y0)])\nRotate this object a-radians clock-wise around its center or the optionally specified pivot." */
static void Smth_rotate(const BoundObject<Object>& self, const radian& angle, const Optional<Point>& origin){
...
static void Smth_set_angles(const BoundObject<Object>& self, const AngleSpan& span){
...
static void Smth_set_text(const BoundObject<Object>& self, const utf8_string& str){
  ObjText* text = dynamic_cast<ObjText*>(self.obj);
  if (!text){
    throw ValueError(space_sep(self.obj->GetType(), "does not support text."));
  }
...
static void Smth_skew(const BoundObject<Object>& self, const coord& skew){
  Object* obj = self.obj;
...
static void Smth_update_settings(const BoundObject<Object>& self, const Settings& s){
