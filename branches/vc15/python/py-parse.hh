...
// Fixme: Do I also need to forward declare specializations?
template<typename T>
...
template<> struct arg_traits<index_t>{static const TypeName name;};
...
template<typename T>
bool parse_item(Optional<T>& item, PyObject* args, int& n, int len, bool allowFlat){
  if (n == len){
...
bool parse_flat(T, PyObject*,int&, int){
  static_assert(TypeDependentFalse<T>::value, "parse_flat not overloaded for type.");
  return false;
}
...
PyObject* build_result(T){
  static_assert(TypeDependentFalse<T>::value, "build_result not overloaded for type.");
  return nullptr;
}
