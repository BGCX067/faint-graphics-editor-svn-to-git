...
FaintDC::~FaintDC(){
  // Required to release unique_ptrs in impl, so that the contained
  // type is visible.
}
...
CairoArc cairo_arc_from_svg_arc(const Point& p0, Radii r, coord xAxisRotation,
  int largeArcFlag, int sweepFlag, const Point& p)
{
...
void FaintDC::Rectangle(const Tri& tri, const Settings& s){
  ...
  m_cr->set_source_tri(tri);
  auto points(points_clockwise(tri));
