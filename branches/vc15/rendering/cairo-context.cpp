...
static pattern_ptr_t faint_cairo_linear_gradient(const LinearGradient& g){
  ...
  for (int i = 0; i != g.GetNumStops().Get(); i++){
    ColorStop stop(g.GetStop(index_t(i)));
...
Bitmap transform_onto(const Bitmap& in_bmpSrc, const IntSize& sz,
...
Bitmap cairo_skew(const Bitmap& src, coord skew_angle, coord skew_pixels){
  ...
  return transform_onto(src, size, Paint(color_transparent_black()),
    Point(0,0),
    [&](cairo_t* cr){
...
Bitmap rotate_bilinear(const Bitmap& src, const Angle& angle, const Paint& bg){
  ...
  return transform_onto(src, adj.size, bg, offset,
    [&](cairo_t* cr){
...
{
  RotationAdjustment adj = get_rotation_adjustment(angle, src.GetSize());
  IntSize size(rounded(floated(adj.size) * scale));
  Point offset(-src.m_w / 2.0, -src.m_h / 2.0);
  return transform_onto(src, rounded(floated(adj.size) * scale), bg,
    offset,
    [&](cairo_t* cr){
      cairo_translate(cr, size.w / 2.0, size.h / 2.0);
      cairo_rotate(cr, angle.Rad());
...
static cairo_matrix_t cairo_linear_matrix_from_tri(const Tri& t, const Angle& angle, bool objectAligned){
  double sx = t.Width();
  double sy = t.Height();
  double x0 = t.P0().x;
  double y0 = t.P0().y;
...
void CairoContext::arc(const Point& c, coord r, coord angle1, coord angle2){
  cairo_arc(m_impl->cr, c.x, c.y, r, angle1, angle2);
}

void CairoContext::arc_negative(const Point& c, coord r, coord angle1, coord angle2){
...
static coord get_ink_height(cairo_ptr_t& cr, font_description_ptr_t& fd){
  auto referenceLayout(manage(pango_cairo_create_layout(cr.get())));
  pango_layout_set_font_description(referenceLayout, fd);
  pango_layout_set_text(referenceLayout.get(), "M", -1);
  PangoRectangle ink;
  PangoRectangle logical;
  pango_layout_get_extents(referenceLayout.get(), &ink, &logical);
  return ink.height / PANGO_SCALE;
}

static layout_ptr_t get_text_layout(cairo_ptr_t& cr, font_description_ptr_t& fd,
  const utf8_string& s)
{
...
void CairoContext::pango_text(const Tri& t, const utf8_string& text,
  const Settings& s)
{
  ...
  translate(Point(0, get_ink_height(m_impl->cr, fd)));

  // Render text as a Cairo path.
...
std::vector<PathPt> CairoContext::get_text_path(const Tri& t, const utf8_string& text, const Settings& s){
  ...
  // Offset to anchor at the top
  Point offset(0, get_ink_height(m_impl->cr, fd));
  translate(offset);
...
// Fixme: Unused function, should use this instead of getting extents of
// a character (like 'M') for calculations.
int get_font_ascent(const Settings& s){
  auto fd(get_font_description(s));
...
void CairoContext::rotate(const Rotation& rotation){
...
void CairoContext::set_source_surface(const Pattern& pattern){
  ...
  m_impl->srcPattern.reset(cairo_pattern_create_for_surface(m_impl->srcSurface.get()));
...
void CairoContext::set_source_gradient(const Gradient& in_gradient){
  if (in_gradient.IsLinear()){
    LinearGradient g(in_gradient.GetLinear());
    if (m_impl->gradientPattern != nullptr){
      m_impl->gradientPattern.reset(nullptr);
      delete m_impl->linearGradient;
      m_impl->linearGradient = nullptr;
    ...
    RadialGradient g(in_gradient.GetRadial());
    if (m_impl->gradientPattern != nullptr){
      m_impl->gradientPattern.reset(nullptr);
      delete m_impl->linearGradient;
      m_impl->linearGradient = nullptr;
