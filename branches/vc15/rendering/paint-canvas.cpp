...
static bool should_draw_raster(const Tool& tool, Layer layer){
...
static bool should_draw_vector(const Tool& tool, Layer layer){
...
static void paint_checkered_background(wxDC& dc, const IntPoint& topLeft, const IntPoint& imageRegionTopLeft, const CanvasGeo& geo, const IntSize& sz){
  const int tileSz = 20;
...
static void paint_fill_background(wxDC& dc, const IntPoint& topLeft, const IntSize& sz, const ColRGB& bg){
  auto bgWx(to_wx(bg));
...
static void paint_objects(FaintDC&& dc, const objects_t& objects, Tool& tool, Overlays& overlays, const PosInfo& posInfo, Layer layer){
  for (auto object : objects){
    object->Draw(dc, posInfo.canvas.GetImage().GetExpressionContext());
  }

  if (!tool.DrawBeforeZoom(layer)){
    // See \ref(draw-before-zoom)
...
void paint_canvas(wxDC& paintDC,
  ...
  const CanvasState& state,
  const IntRect& updateRegion,
  const std::weak_ptr<Bitmap>& weakBitmapChimera,
  ...
  int objectHandleWidth)
{
  ...
  paint_objects(
    ...
    active.GetObjects(), tool, overlays, posInfo, layer);
