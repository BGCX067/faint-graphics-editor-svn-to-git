...
faint::coord compute_row_height( FaintDC& dc, const Settings& settings ){
  return dc.TextSize(faint::utf8_string("M"), settings).h;
}

faint::coord ComputeRowHeight( const Settings& settings ){
  faint::Bitmap dummy( IntSize(10, 10) ); // Fixme: Bitmap-creation like this is duplicated all over this file.
  FaintDC dc( dummy );
...
text_lines_t split_text_buffer( const TextBuffer& buffer, const Settings& settings, const max_width_t& maxWidth ){
  faint::Bitmap dummy(IntSize(10, 10));
  FaintDC dc( dummy );
...
void DrawSelection( FaintDC& dc, const TextBuffer& textBuf, const text_lines_t& lines, const Settings& settings, const Tri& tri, const Settings& highlightSettings, const faint::coord ht ){
  ...
  Size size0 = dc.TextSize( lines[ row_s0 ].second.substr(0, char_s0), settings );
  Size size1 = dc.TextSize( lines[ row_s1 ].second.substr(0, char_s1), settings );

  for ( size_t row = row_s0; row <= row_s1; row++ ){
    faint::coord dx = row * ht * sin( -tri.Angle() );
    faint::coord dy = row * ht * cos( tri.Angle() ) - 1;
    Size rowSize = dc.TextSize( lines[ row ].second, settings );
...
void ObjText::Init(){
  ...
  m_highlightSettings.Set( ts_FgCol, faint::DrawSource(highlightColor) );
  m_highlightSettings.Set( ts_BgCol, faint::DrawSource(highlightColor) );
...
LineSegment ObjText::ComputeCaret( const FaintDC& dc, const Tri& tri, faint::coord ht, const text_lines_t& lines ){
  ...
  Size textSize = dc.TextSize( line.substr(0, caretChar), m_settings );
  textSize.h = ht;
...
faint::coord ObjText::BaselineOffset() const{
  faint::Bitmap dummy(IntSize(10, 10));
  FaintDC dc( dummy );
  TextMeasures<Rect> rects( dc.TextExtents(faint::utf8_string(1, faint::utf8_char("M")), m_settings ) );
  return rects.ink.h;
}

size_t ObjText::CaretPos( const Point& posUnaligned ) const{
  faint::Bitmap bmp(IntSize(10,10));
  FaintDC dc( bmp );
  ...
  std::vector<int> extents(dc.CumulativeTextWidth(lines[row].second, m_settings));
...
void ObjText::DrawMask( FaintDC& dc ){
  ...
  text_lines_t lines = split_text_buffer( m_textBuf, m_settings, max_width_t(GetTri().Width()) );
  m_rowHeight = compute_row_height( dc, m_settings );
  const faint::coord ht = m_rowHeight;
...
Rect ObjText::GetAutoSizedRect() const {
  ...
  Size extents(0,0);
  for ( size_t i = 0; i!= lines.size(); i++ ){
    extents = dc.TextSize( lines[i].second, m_settings );
