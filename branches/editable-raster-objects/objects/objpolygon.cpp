...
ObjPolygon::ObjPolygon( const Points& points, const Settings& settings )
  : Object( &s_TypePolygon, points.GetTri(),
    with_point_editing(settings, start_enabled(false)) ),
    m_points( points )
{}
...
void ObjPolygon::Draw( FaintDC& dc ){
  Tri tri(GetTri());
  dc.Polygon( tri, m_points.GetPointsDumb(tri),
    m_settings );
}
...
IntRect ObjPolygon::GetRefreshRect(){
  return floored( inflated(GetRect(), m_settings.Get(ts_LineWidth) * 2 ) );
}
