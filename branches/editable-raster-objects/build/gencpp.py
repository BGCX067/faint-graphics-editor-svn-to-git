...
def generate_stringtoint_function( setting, item ):
    ...
    for line in Texts.template_stringtoint_cc:
       ...
       if line.find( "$string_to_int" ) != -1:
           ...
           Files.targetCC.write(line)
           Files.targetCC.write('  if ( strcmp(s_value, "%s" ) == 0 ){\n    value = %s;\n  }\n' %(key, get_value( stringmap, key ) ) )
           # Each else if
           for key in keys[1:]:
...
def generate_stringtoint_settings_function( setting, item ):
    ...
    for line in Texts.template_settings_stringtoint:
        ...
        if line.find( "$string_to_int" ) != -1:
            ...
            Files.targetSettingsHH.write( line )
            Files.targetSettingsHH.write('  if ( strcmp(s_value, "%s" ) == 0 ){\n    value = %s;\n  }' % (keys[0], get_value(stringmap, keys[0]) ) )
            for key in keys[1:]:
                str = '  else if ( strcmp(s_value, "%s" ) == 0 ){\n    value = %s;\n  }\n' %(key, get_value(stringmap, key ) )
            ...
            Files.targetSettingsHH.write('     return faint::setter_fail;\n')
...
def write_cpp_value_string_impl( out, settings ):
    ...
    out.write("std::string value_string(const IntSetting& setting, int value){\n");
...
def write_cpp_header( file_name, settings ):
    ...
    out.write("std::string setting_name(const UntypedSetting&);\n");
    out.write("std::string setting_name_pretty(const UntypedSetting&);\n");
    ...
    out.write("std::string value_string(const IntSetting&, int value);\n");
...
def write_setting_name_impl( out, settings, pretty ):
    if not pretty:
        out.write('std::string setting_name( const UntypedSetting& s ){\n')
    else:
        out.write('std::string setting_name_pretty( const UntypedSetting& s ){\n')
