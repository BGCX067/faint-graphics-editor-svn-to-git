        ...
        for folder in folders:
            for file in faint_info.enumerate_cpp( folder ):
                ...
                if not os.path.isfile( objFile ) or do_rebuild or ( os.path.isfile( objFile ) and os.path.getmtime( objFile ) < os.path.getmtime( cppFile ) ):
                    to_compile.append( os.path.join(folder, file ) )

    if len(to_compile) > 0:
        compile( to_compile, faintRoot, include, out, err, objRoot, wx_root, version=opts.version, debug=opts.debug, num_parallell=parallell_compiles )
