...
class Image:
    ...
    def __init__( self, filename ):
        ...
        if not os.path.exists(relative_path):
            fail_with_message("Missing image: %s" % relative_path)
    def to_html( self, **kwArgs ):
...
class Graphic:
    def __init__( self, filename ):
        ...
        if not os.path.exists(relative_path):
            fail_with_message("Missing graphic: %s" % relative_path)
    def to_html( self, **kwArgs ):
...
class Reference:
    ...
    def to_html( self, **kwArgs ):
        ...
        if self.target_label is None:
            fail_with_message("Undefined label: %s" % self.name)
...
class ExternalReference:
    def __init__( self, url ):
        if not url.startswith("http://"):
            fail_with_message('External url not prefixed by http://. "%s"' % url)
        self.url = url
...
class PythonSample:
    ...
    def to_html( self, **kwArgs ):
        ...
        try:
            ...
            fail_with_message('PythonSample %s missing #start-comment' % self.filename)
...
def run(force=False):
    ...
    if not force and not _need_generate(helpSource, helpRoot, source_pages, ["contents.dat"]):
        print " Up to date."
        return
