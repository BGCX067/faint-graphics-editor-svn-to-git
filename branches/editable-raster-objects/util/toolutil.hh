...
ColorSetting fg_or_bg( const CursorPositionInfo& );
...
typedef Unique<bool, category_toolutil, 0> hidden_fill;

// Returns the DrawSource under the given position. Includes objects
// if in the object layer.  Must not be called with positions outside
// the Canvas!
faint::DrawSource get_hovered_draw_source( const CursorPositionInfo&,
  const hidden_fill& );
...
// object.
ObjText* hovered_selected_text(const CursorPositionInfo&, SearchMode searchMode=SearchMode::exact_object );
...
bool outside_canvas_by( const CursorPositionInfo&, int pixels );
