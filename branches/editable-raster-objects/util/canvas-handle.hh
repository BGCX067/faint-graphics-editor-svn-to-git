...
enum class HandleDirection{ UP_DOWN, LEFT_RIGHT, DIAGONAL };
HandlePos opposite_handle_pos(HandlePos);
HandleDirection handle_direction(HandlePos);

class CanvasResizeHandle{
public:
  CanvasResizeHandle( HandlePos, const IntSize& canvasSize, const Geometry& geo );
...
Optional<CanvasResizeHandle> canvas_handle_hit_test( const IntPoint&, const IntSize& canvasSize, const Geometry& );
