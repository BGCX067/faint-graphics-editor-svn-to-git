...
typedef FaintID<110> PatternId;
typedef std::map<PatternId, int> ref_map_t;
typedef std::map<PatternId, Bitmap> pat_map_t;
...
public:
  ...
  bool operator<(const PatternRef&) const;
private:
  PatternRef operator=(const PatternRef&); // Prevent assignment
  PatternId m_id;
};
...
PatternHandler& get_pattern_handler(){
...
Pattern::Pattern( const Bitmap& bmp )
  : m_anchor(0,0),
    m_objectAligned(false),
    m_ref(new PatternRef(get_pattern_handler().Add(bmp)))
{}

Pattern::Pattern( const Bitmap& bmp, const IntPoint& anchor, const object_aligned_t& objectAligned )
  : m_anchor(anchor),
...
bool Pattern::operator==( const Pattern& other ) const{
  return ( *m_ref == *other.m_ref && m_anchor == other.m_anchor );
}
