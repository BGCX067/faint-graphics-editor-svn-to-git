...
RasterSelectionOptions::RasterSelectionOptions( bool in_mask, const faint::DrawSource& in_bg, bool in_alpha )
  : alpha(in_alpha),
    bg(in_bg),
    mask(in_mask)
{}
...
bool RasterSelectionState::Floating() const{
  return floating;
}
...
RasterSelection::RasterSelection()
  : m_options(false, faint::DrawSource(faint::Color(255,255,255)), false)
{}
...
void RasterSelection::BeginFloat(const faint::Bitmap& src, const copy_selected& copy ){
  ...
  SetFloatingBitmap(sub_bitmap(src, m_state.rect), m_state.rect.TopLeft());
}
...
void RasterSelection::DrawFloating(FaintDC& dc) const{
  ...
  dc.Bitmap( m_state.floatingBmp, floated(m_state.rect.TopLeft()), bitmap_mask_settings(m_options.mask, m_options.bg, m_options.alpha) );
}
...
const faint::DrawSource& RasterSelection::GetBackground() const{
  return m_options.bg;
}
...
void RasterSelection::SetBackground( const faint::DrawSource& bg ){
  m_options.bg = bg;
}
...
RasterSelectionOptions raster_selection_options( const Settings& s ){
  return RasterSelectionOptions(
    masked_background(s), s.Get(ts_BgCol), alpha_blending(s));
}

void update_mask( bool enableMask, const faint::DrawSource& bg, bool enableAlpha, RasterSelection& selection ){
