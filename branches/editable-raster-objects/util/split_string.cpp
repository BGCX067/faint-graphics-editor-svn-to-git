...
faint::utf8_char softBrk(faint::downwards_arrow_with_tip_leftwards);
#else
...
void split_line( FaintDC& dc, const faint::utf8_string& string, faint::coord maxWidth, text_lines_t& result, const Settings& settings ){
  size_t wordStart = 0;
  size_t wordEnd = 0;
  ...
  do {
    ...
    if ( !line.empty() && getw( dc, line + faint::space + word, settings ) > maxWidth ){
...
text_lines_t split_str( FaintDC& dc, const faint::utf8_string& string, const Settings& settings, const max_width_t& maxWidth ){
  size_t lineEnd = 0;
  size_t lineStart = 0;
  ...
  do {
    ...
    if ( maxWidth.NotSet() || getw( dc, string.substr( lineStart, lineEnd - lineStart ), settings ) < maxWidth.Get() ){
      if ( softBreak ){
