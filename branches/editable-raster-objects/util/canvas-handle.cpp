...
CanvasResizeHandle::CanvasResizeHandle( HandlePos handlePos, const IntSize& canvasSize, const Geometry& geo )
...
Cursor CanvasResizeHandle::GetCursor() const{
  if ( m_handlePos == HandlePos::TOP_LEFT || m_handlePos == HandlePos::BOTTOM_RIGHT ){
    return Cursor::RESIZE_NW;
  }
  else if ( m_handlePos == HandlePos::TOP_RIGHT || m_handlePos == HandlePos::BOTTOM_LEFT ){
    return Cursor::RESIZE_NE;
  }
  else if ( m_handlePos == HandlePos::LEFT_SIDE || m_handlePos == HandlePos::RIGHT_SIDE ){
    return Cursor::RESIZE_WE;
  }
  else if ( m_handlePos == HandlePos::TOP_SIDE || m_handlePos == HandlePos::BOTTOM_SIDE ){
...
IntPoint CanvasResizeHandle::GetPos() const{
  switch (m_handlePos){
  case HandlePos::TOP_LEFT: return IntPoint(0,0);
  case HandlePos::TOP_RIGHT: return IntPoint(m_canvasSize.w, 0);
  case HandlePos::BOTTOM_LEFT: return IntPoint(0, m_canvasSize.h);
  case HandlePos::BOTTOM_RIGHT: return IntPoint(m_canvasSize.w, m_canvasSize.h);
  case HandlePos::TOP_SIDE: return IntPoint((m_canvasSize.w - 1)/2, 0);
  case HandlePos::BOTTOM_SIDE: return IntPoint( (m_canvasSize.w - 1)/2, m_canvasSize.h - 1);
  case HandlePos::LEFT_SIDE: return IntPoint( 0, (m_canvasSize.h - 1)/2);
  case HandlePos::RIGHT_SIDE: return IntPoint( m_canvasSize.w - 1, (m_canvasSize.h - 1)/2);
  case HandlePos::END: assert(false); return IntPoint(0,0);
  }
  assert(false);
  return IntPoint(0,0);
}
...
IntRect CanvasResizeHandle::GetRect() const{
  const int xOffset = m_geo.left_border / 2 - g_canvasHandleSz.w / 2;
  const int yOffset = m_geo.top_border / 2 - g_canvasHandleSz.h / 2;

  const int left = -m_geo.x0 + xOffset;
  const int top = -m_geo.y0 + yOffset;
  ...
  const int right = m_geo.left_border + width - m_geo.x0 + xOffset;
  const int bottom = m_geo.top_border + height - m_geo.y0 + yOffset;
  ...
  if ( m_handlePos == HandlePos::TOP_LEFT ){
    ...
    return IntRect(IntPoint((int)m_geo.left_border + width / 2 - m_geo.x0 - ch_w / 2, top),
      g_canvasHandleSz);
  }
  else if ( m_handlePos == HandlePos::BOTTOM_SIDE ){
    const int ch_w = g_canvasHandleSz.w;
    return IntRect(IntPoint(m_geo.left_border + width / 2 - m_geo.x0 - ch_w / 2, bottom),
      g_canvasHandleSz);
  }
  else if ( m_handlePos == HandlePos::RIGHT_SIDE ){
    const int ch_h = g_canvasHandleSz.h;
    return IntRect(IntPoint( right, m_geo.top_border + height / 2 - m_geo.y0 - ch_h / 2), g_canvasHandleSz);
  }
  else if ( m_handlePos == HandlePos::LEFT_SIDE ){

    const int ch_h = g_canvasHandleSz.h;
    return IntRect(IntPoint(left,m_geo.top_border + height / 2 - m_geo.y0 - ch_h / 2),
        g_canvasHandleSz);
  }
  else {
    assert( false );
...
CanvasResizeHandle CanvasResizeHandle::Opposite() const{
  return CanvasResizeHandle(opposite_handle_pos(m_handlePos), m_canvasSize, m_geo);
}

Optional<CanvasResizeHandle> canvas_handle_hit_test( const IntPoint& mousePos, const IntSize& canvasSize, const Geometry& geo){
  for ( HandlePos hp : iterable<HandlePos>() ){
