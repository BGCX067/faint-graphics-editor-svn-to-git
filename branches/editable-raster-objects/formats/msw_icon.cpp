...
std::string error_cursor_is_icon(){
  return "Error: Cursor file contains icons.";
}
...
uint read_2u( faint::BinaryReader& f ){
  char data[2];
  f.read(data,2);
  return to_uint(data[0], data[1]);
}

uint read_4u( faint::BinaryReader& f ){
  char data[4];
  f.read(data, 4);
  return to_uint(data[0], data[1], data[2], data[3]);
}

int read_4( faint::BinaryReader& f ){
  char data[4];
  f.read(data, 4);
  return to_int(data[0], data[1], data[2], data[3]);
}

void write_1b(faint::BinaryWriter& out, uint v){
  char data((char)v);
  out.write(&data, 1);
}

void write_2b(faint::BinaryWriter& out, uint v ){
  char data[2];
...
void write_4b(faint::BinaryWriter& out, uint v ){
...
IntSize read_bmp_size(faint::BinaryReader& f){
...
IntSize read_ico_size(faint::BinaryReader& f){
...
IconDirEntry parse_icon_dir_entry( faint::BinaryReader& f ){
  IconDirEntry icon;
  ...
  icon.bpp = to_uint(data[0], data[1]);
...
bool peek_png_signature( faint::BinaryReader& f ){
...
void test_icon_dir(const IconDir& dir, bool icon ){
...
void test_icon_dir_entry(size_t iconNum, const IconDirEntry& icon, bool cursor ){
...
void test_icon_dir_entries(std::vector<IconDirEntry>& entries, bool cursor){
  for ( size_t i = 0; i != entries.size(); i++ ){
    test_icon_dir_entry(i, entries[i], cursor);
  }
}

BitmapHeader parse_bitmap_header( faint::BinaryReader& f ){
  BitmapHeader h;
...
IconDir parse_icon_dir( faint::BinaryReader& f ){
  IconDir d;
...
std::vector<IconDirEntry> parse_icon_dir_entries(faint::BinaryReader& f, size_t numIcons ){
...
faint::Bitmap ico_parse_1bpp( faint::BinaryReader& f, const IntSize& bitmapSize ){
  ...
  faint::Color c0(palette[2],palette[1],palette[0],255);
  faint::Color c1(palette[6],palette[5],palette[4],255);
  ...
  for ( int y = 0; y != sz.h; y++ ){
    for ( int x = 0; x != sz.w; x++ ){
      ...
      if ( ( value & ( 1 << ( 7 - x % 8 ) ) ) ){
        put_pixel_raw(bmp, x, bmp.m_h - y - 1, faint::Color(0,0,0,0));
      }
    }
  }
...
faint::Bitmap ico_parse_32bpp_BI_RGB( faint::BinaryReader& f, const IntSize& bitmapSize ){
  int bypp = 4;
  ...
  for ( int y = 0; y != sz.h; y++ ){
    for ( int x = 0; x != sz.w; x++ ){
      ...
      put_pixel_raw(bmp, x, bmp.m_h - y - 1, faint::color_from_uints(r,g,b,a));
    }
  }
  delete[] pixelData;
  return bmp;
...
faint::Bitmap ico_parse_png( faint::BinaryReader& f, size_t len ){
...
void write_32bpp_BI_RGB( faint::BinaryWriter& out, const faint::Bitmap& bmp ){
  // The size from the bmp-header. May have larger height than the size
  // in the IconDirEntry. (Fixme: Why?)
...
void load_icon( const FilePath& filename, ImageProps& props ){
  try {
    ...
    if ( !f.good() ){
      props.SetError(error_open_file(filename.ToAscii())); // Fixme: Will fail for "wide" filenames
      return;
    }
    ...
    for ( size_t i = 0; i != iconEntries.size(); i++ ){
      ...
      if ( png_compressed ){
        ...
        if ( bmpHeader.bpp == 32 ){
          ...
          faint::Bitmap bmp(ico_parse_1bpp(f, icon.size));
          if ( !f.good() ){
            throw LoadIconError(error_bmp_data(i));
          }
          assert(faint::bitmap_ok(bmp));
  ...
  try {
    ...
    for ( size_t i = 0; i != iconEntries.size(); i++ ){
      ...
      if ( png_compressed ){
        ...
        props.AddFrame(bmp, FrameInfo(hotSpot));
      }
      else {
        BitmapHeader bmpHeader(parse_bitmap_header(f));
        if ( !f.good() ){
...
typedef std::vector<faint::Bitmap> bmp_vec_t;
...
IconDir create_icon_dir( const bmp_vec_t& bitmaps, int imageType ){
...
std::vector<IconDirEntry> create_cursor_dir_entries(const bmp_vec_t& bitmaps, const std::vector<IntPoint>& hotSpots ){
  std::vector<IconDirEntry> v;
  int offset = g_iconDirLen + bitmaps.size() * g_iconDirEntryLen;
  for ( size_t i = 0; i != bitmaps.size(); i++ ){
    assert( i < bitmaps.size() );
...
SaveResult save_cursor( const FilePath& filePath,  const bmp_vec_t& bitmaps, const std::vector<IntPoint>& hotSpots ){
  ...
  for ( size_t i = 0; i != bitmaps.size(); i++ ){
    const Bitmap& bmp(bitmaps[i]);
    if ( bmp.m_w > 256 || bmp.m_h > 256 ){
      return SaveResult::SaveFailed("Maximum size for cursors is 256x256");
    }
  }
