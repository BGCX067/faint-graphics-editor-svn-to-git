...
faint::Color ColorDataObject::GetColor() const{
  return m_color;
}
...
bool ColorDataObject::GetDataHere( const wxDataFormat& format, void *buf ) const{
  if ( format == wxDataFormat("FaintColor") ){
    ...
    wxString dragText(str_smart_rgba(m_color));
...
bool ColorDataObject::SetData( const wxDataFormat& format, size_t len, const void* buf ){
  if ( format == wxDataFormat("FaintColor") && len == sizeof(ColorStruct) ){
    ColorStruct c;
    memcpy( &c, buf, len );
    m_color = faint::color_from_ints( c.r, c.g, c.b, c.a );
    return true;
  }
  return false;
}
...
wxDragResult ColorDropTargetImpl::OnData( wxCoord x, wxCoord y, wxDragResult ){
  ...
  wxDragResult result = m_target->OnDropColor( IntPoint(x, y), colorObj->GetColor() );
