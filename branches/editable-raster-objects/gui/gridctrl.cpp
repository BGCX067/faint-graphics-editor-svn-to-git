...
DragValueCtrl* grid_text( wxWindow* parent, wxSizer* sizer, winvec_t& showhide ){
  DragValueCtrl* text = new DragValueCtrl(parent, IntRange(min_t(1)), description_t("Drag to adjust grid spacing. Right-Click to disable grid."));
    ...
    m_enabled(false)
{
  ...
  m_txtCurrentSize->Connect(wxID_ANY, wxEVT_RIGHT_DOWN, wxMouseEventHandler(GridCtrl::OnRightClickDrag), 0, this);
  SetSizerAndFit( m_sizer );
}

GridCtrl::~GridCtrl(){
  m_txtCurrentSize->Disconnect(wxID_ANY, wxEVT_RIGHT_DOWN);
}
