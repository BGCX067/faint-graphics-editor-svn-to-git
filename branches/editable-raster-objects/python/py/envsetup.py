...
def pick_color_fg():
    try:
        ifaint.set_fg(ifaint.get_pixel(*ifaint.get_mouse_pos()))
    finally:
        return

def pick_color_bg():
    try:
        ifaint.set_bg(get_pixel(*get_mouse_pos()))
    finally:
        return
...
ifaint.file_browse_cmd_msie = lambda f:'explorer /select,"%s"' % f
...
ifaint.blur = _forwarder(_c.blur)
...
ifaint.get_pixel = _active(_c.get_pixel)
...
bindk(key.space, NamedFunc( "Select or Traverse", lambda: (ifaint.select_top_object() if ifaint.get_layer() == 1
                          else ifaint.scroll_traverse())))
try:
    import py.formatsvg as formatsvg
    ifaint.add_format( formatsvg.load, formatsvg.save, "Scalable Vector Graphics(*.svg)", "svg")
    del formatsvg
except Exception, e:
    ifaint.the_error = str(e)
...
def snowman():
    Text((20,20,40,40),u'\u2603', ifaint.get_settings())

ifaint.grid = ifaint.active_grid()
...
def toggle_flag_pixel():
    ...
    if _flagged.has_key(image) and _flagged[image] == pos:
        del _flagged[image]
...
def anchor_flagged():
    ...
    """
    anchor( _flagged.keys(), _flagged.values() )
    clear_flagged()
...
def faint_resolve( item_str ):
    """Test function for highlighting objects indicated in the
    interpreter.  Currently totally fake

    """
    try:
        object = _console.locals[item_str]
        if object.__class__ == ifaint.Something:
            object.set_fg((255,0,0))
    except:
        pass
