...
void select_word_at_pos( ObjText* textObject, const Point& pos ){
...
TaskResult EditText::Commit(Layer layerType){
  if ( m_active ){
    EndEntry();
  }
  if ( m_newTextObject ){
  ...
  const faint::utf8_string& newText(m_textObject->GetTextBuffer()->get());
  if ( newText == m_oldText ){
    // Text unchanged, create no no command.
    return TASK_CHANGE;
  }
  m_command.Set( new TextEntryCommand( m_textObject, New(newText), Old(m_oldText) ) );
  return TASK_COMMIT_AND_CHANGE;
}
...
void EditText::Draw( FaintDC& dc, Overlays& overlays, const Point& ){
...
bool EditText::DrawBeforeZoom(Layer layer) const{
...
Cursor EditText::GetCursor( const CursorPositionInfo& info ) const{
...
TaskResult EditText::LeftDoubleClick( const CursorPositionInfo& info ){
...
TaskResult EditText::LeftDown( const CursorPositionInfo& info ){
  ...
  m_newTask.Set(new SelectText(m_textObject, m_newTextObject, info.pos, m_settings));
  return TASK_PUSH;
}
