...
static std::vector<Point> get_corners( const Object* object, size_t pointIndex ){
...
MovePointTask::MovePointTask(Object* object, size_t pointIndex, MovePointMode mode )
  : m_mode(mode),
    m_object(object),
...
void MovePointTask::Draw( FaintDC& dc, Overlays& overlays, const Point& ){
...
bool MovePointTask::DrawBeforeZoom(Layer) const{
  return false;
}
...
Cursor MovePointTask::GetCursor( const CursorPositionInfo& ) const{
