...
public:
  ...
  virtual void Bind( int key, int modifiers, bool global=false ) = 0; // Fixme: Move to Python-interfacing class.
  virtual bool Bound( int key, int modifiers ) const = 0; // Fixme: Move to Python-interfacing class.
  virtual bool BoundGlobal( int key, int modifiers ) const = 0; // Fixme: Move to Python-interfacing class.
  ...
  virtual StatusInterface& GetStatusInfo() = 0; // Fixme: Remove
  ...
  virtual CanvasInterface* Load( const faint::FilePath&, const change_tab& ) = 0;
  virtual void Load( const faint::FileList& ) = 0;
  virtual CanvasInterface* LoadAsFrames( const faint::FileList&, const change_tab& ) = 0;
  ...
  // Run a Faint-command from a Python interface function
  virtual void PythonRunCommand( CanvasInterface*, Command*, const FrameId& ) = 0; // Fixme: Move to Python-interfacing class.

  // Queues the files for loading, but returns immediately
  virtual void QueueLoad( const faint::FileList& ) = 0;
  virtual void Quit() = 0;
  ...
  virtual void SetPalette( const faint::DrawSourceMap& ) = 0;
  virtual void SetTransparencyStyle(const TransparencyStyle&) = 0;
  virtual void Show( CommandDialog& ) = 0;
