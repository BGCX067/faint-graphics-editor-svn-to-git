...
static void line_status_bar( StatusInterface& status, const Point& p1, const Point& p2, int modifiers, bool subPixel ){
  ...
  if ( subPixel ){
    status.SetText(str_line_status_subpixel( p1, p2 ), 0);
  }
  else{
    status.SetText(str_line_status( floored(p1), floored(p2)), 0);
  }
}
...
Command* LineTool::CreateCommand( Layer layer ){
...
void LineTool::Draw( FaintDC& dc, Overlays&, const Point& ){
  if ( m_active ){
    dc.PolyLine( tri_from_points(m_points), m_points,
      clean_line_object_settings(m_settings) );
  }
}

bool LineTool::DrawBeforeZoom( Layer layer ) const{
...
Cursor LineTool::GetCursor( const CursorPositionInfo& ) const{
...
IntRect LineTool::GetRefreshRect( const IntRect& visible, const Point& ) const{
  if ( m_points.size() == 2 ){
    ...
    return floored( inflated( Rect( p0, p1 ), extraWidth ) );
  }
  else {
    return visible; // Fixme: Use a more efficient polyline refresh rect
  }
}
...
ToolResult LineTool::UpdatePoints( const CursorPositionInfo& info, bool first, bool click ){
  ...
  line_status_bar( *info.status, m_points[0], m_points[1], info.modifiers, m_settings.Get(ts_AntiAlias) );
  return TOOL_DRAW;
}
